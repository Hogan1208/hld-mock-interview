  ...
  q:{
    ...
    olap:[
      {l:"medium",tag:"concept",q:"Which store, and what's the schema?",turns:[
        {who:"intv",text:"What store holds the aggregates, and what's the schema for near-real-time advertiser queries?"},
        {who:"cand",text:"A <strong>column-oriented OLAP store</strong> built for time-series rollups and fast group-by scans — Druid, Pinot, or ClickHouse. Schema is a wide fact keyed by time + entity: <code>(minute, adId, campaignId, advertiserId)</code> → <code>count</code>, with dims like geo/device as columns.<span class='eg'>rollup: minute → hour → day pre-aggregated, partitioned by time</span>Time-partitioned segments make range scans and retention/expiry cheap."},
        {who:"intv",text:"Why not a KV store like DynamoDB keyed by <code>(adId, minute)</code>? Point reads are fast there."},
        {who:"cand",text:"Point reads yes, but advertiser queries are <strong>range + group-by</strong> — sum over the last 24h grouped by campaign or geo. A KV store would force thousands of point reads plus app-side summation per query. A column store with time partitioning and pre-rolled aggregates answers those with one segment scan in ms, and only stores columns the query touches. The access pattern is analytical, so the store should be analytical."},
      ],resources:[
        {title:"Hello Interview — Ad Click Aggregator",url:"https://www.hellointerview.com/learn/system-design/answer-keys/ad-click-aggregator"},
