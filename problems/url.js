...
window.DATA['url'] = {
  ...
  deepFlows:[
    {id:"shorten-e2e",name:"Shorten URL",summary:"**POST /shorten** → gateway throttle → validate → **lease/base62 key** → **conditional INSERT into <code>urls</code> by <code>short_key</code>** → quorum replication → warm Redis → return the short link.",steps:[
      {node:"client",title:"Client submits a long URL",snap:{"cap":"Nothing durable exists yet. The candidate mapping is routed to <strong>urls shard p17</strong> by <code>hash(15ftgG)</code>, but no row is safe until the conditional insert and quorum replication finish.","tables":[{"name":"urls (target shard p17)","cols":["short_key","long_url","user_id","created_at","expires_at","is_custom"],"rows":[{"c":["15ftgG","https://example.com/very/long/path?ref=x","42","<em>not written</em>","(null)","false"],"hi":1,"tag":"planned"},{"c":["my-sale","https://shop.example.com/summer-sale","7","2026-07-20 09:12:00","2026-08-01 00:00:00","true"]}],"note":"PK = short_key"}]},narrate:"The create path is small but correctness-sensitive: a flaky mobile client may retry, and two users may race for the same custom alias. The client sends the destination, optional alias/expiry, and an idempotency key so a timed-out request can be replayed safely.",details:[
      ...
      {node:"lb",title:"Gateway gates the create",snap:{"cap":"Still no state change: the edge has authenticated and rate-limited the write before it can burn a key or storage. Rejecting here prevents abuse from reaching key generation and quorum writes.","tables":[{"name":"urls (target shard p17)","cols":["short_key","long_url","user_id","created_at","expires_at","is_custom"],"rows":[{"c":["15ftgG","https://example.com/very/long/path?ref=x","42","<em>not written</em>","(null)","false"],"hi":1,"tag":"gated"}]}]},narrate:"The LB terminates TLS and routes to any healthy shortener instance. The gateway treats creates differently from redirects: authenticated, validated early, and rate-limited because create spam burns keys and storage.",details:[
      ...
      {node:"svc",title:"Service validates and dedups",snap:{"cap":"URL and expiry validation pass, and the idempotency lookup is a miss, so the service may allocate a new key. The core mapping table is still unchanged until the later <code>urls</code> insert.","tables":[{"name":"urls (shard p17)","cols":["short_key","long_url","user_id","created_at","expires_at","is_custom"],"rows":[{"c":["15ftgG","https://example.com/very/long/path?ref=x","42","<em>not written</em>","(null)","false"],"hi":1,"tag":"validated"}],"note":"no reverse long_url index is consulted"}]},narrate:"The shortener service is stateless. It validates the URL and expiry, then uses the idempotency key only to make client retries return the same <code>short_key</code>; it does **not** globally dedupe long URLs by default because a reverse index would double write cost on a 180B-row store.",details:[
      ...
      {node:"key",title:"Mint the short key",snap:{"cap":"Instance <code>svc-7f3a</code> burns id <code>1000000007</code> from its leased block and base62-encodes it to <code>15ftgG</code>. The allocator row is already leased; unused ids can be skipped on crash.","tables":[{"name":"id_ranges","cols":["range_start","range_end","instance_id","assigned_at"],"rows":[{"c":["1000000000","1000999999","svc-7f3a","2026-07-22 09:59:50"],"hi":1,"tag":"owns next id"},{"c":["1001000000","1001999999","svc-b12c","2026-07-22 10:01:14"]}]}]},narrate:"For generated links, the service burns the next integer from a locally leased block and base62-encodes it. The integer is already unique, so base62 is collision-free by construction; no hash-collision retry loop is needed.",details:[
      ...
      {node:"db",title:"Insert the mapping on its shard",snap:{"cap":"The durable mapping is created on <strong>urls shard p17</strong> with <code>short_key</code> as the primary key. This conditional insert is the first mutation that makes the short URL exist.","tables":[{"name":"urls (shard p17 leader)","cols":["short_key","long_url","user_id","created_at","expires_at","is_custom"],"rows":[{"c":["15ftgG","https://example.com/very/long/path?ref=x","42","2026-07-22 10:00:00","(null)","false"],"hi":1,"tag":"inserted"},{"c":["9kQ2aZ","https://docs.example.com/guide","(null)","2026-07-22 11:30:00","(null)","false"]}],"note":"hash(15ftgG) → p17"}]},narrate:"The mapping store is a hash-partitioned KV/wide-column store. The sharding key is <code>short_key</code>, so every create/read is a single-partition point operation; custom aliases use the same path but require a conditional put.",details:[
      ...
      {node:"replica",from:"db",title:"Replicate before ack",snap:{"cap":"Ack waits for quorum: primary plus one follower have the exact <code>urls</code> row durably. The third replica may lag, but one node loss after the ack cannot lose <code>15ftgG</code>.","tables":[{"name":"urls (p17 primary)","cols":["short_key","long_url","user_id","created_at","expires_at","is_custom"],"rows":[{"c":["15ftgG","https://example.com/very/long/path?ref=x","42","2026-07-22 10:00:00","(null)","false"],"hi":1,"tag":"fsynced"}]},{"name":"urls (p17 follower A)","cols":["short_key","long_url","user_id","created_at","expires_at","is_custom"],"rows":[{"c":["15ftgG","https://example.com/very/long/path?ref=x","42","2026-07-22 10:00:00","(null)","false"],"hi":1,"tag":"acked"}]},{"name":"urls (p17 follower B)","cols":["short_key","long_url","user_id","created_at","expires_at","is_custom"],"rows":[{"c":["15ftgG","<em>replicating</em>","42","2026-07-22 10:00:00","(null)","false"],"tag":"lagging"}]}]},narrate:"Each shard is a replica group of at least three nodes across AZs. A create is acknowledged after a quorum, typically 2/3, has persisted the row; create latency can pay a few milliseconds because reads dominate the product.",details:[
      ...
      {node:"cache",title:"Warm Redis best-effort",snap:{"cap":"Redis is warmed only after the mapping is durable. If this best-effort write is lost, a redirect miss can still rebuild from <code>urls</code>; the cache is not the source of truth.","tables":[{"name":"Redis cache","cols":["cache_key","long_url","expires_at","ttl"],"rows":[{"c":["url:15ftgG","https://example.com/very/long/path?ref=x","(null)","30d+jitter"],"hi":1,"tag":"warmed"}]},{"name":"urls (shard p17)","cols":["short_key","long_url","user_id","created_at","expires_at","is_custom"],"rows":[{"c":["15ftgG","https://example.com/very/long/path?ref=x","42","2026-07-22 10:00:00","(null)","false"],"tag":"source"}]}]},narrate:"After the durable write, the service warms Redis with the immutable mapping. Cache warming is an optimization, not the source of truth; if the service crashes here, the first redirect simply misses and populates read-through.",details:[
      ...
      {node:"client",from:"svc",title:"Return the short URL",snap:{"cap":"The client receives a durable, unique key. A retry with the same idempotency key should return this existing <code>15ftgG</code> mapping, not allocate a second key.","tables":[{"name":"urls (shard p17)","cols":["short_key","long_url","user_id","created_at","expires_at","is_custom"],"rows":[{"c":["15ftgG","https://example.com/very/long/path?ref=x","42","2026-07-22 10:00:00","(null)","false"],"hi":1,"tag":"committed"}]},{"name":"Redis cache","cols":["cache_key","long_url","expires_at","ttl"],"rows":[{"c":["url:15ftgG","https://example.com/very/long/path?ref=x","(null)","30d+jitter"],"hi":1,"tag":"ready"}]}]},narrate:"Now the API can respond: the key is unique, the <code>urls</code> row is quorum-durable, and the cache is likely warm. A retry with the same idempotency key should return this same response.",details:[
    ...
    {id:"redirect-e2e",name:"Redirect click",summary:"**GET /{short_key}** → edge/service route → Redis hit or read-replica miss fill → expiry check → **302 vs 301 decision** → async click event → browser follows <code>Location</code>.",steps:[
      {node:"client",title:"Browser follows the short link",snap:{"cap":"Read-only step: the browser asks for <code>15ftgG</code>. The mapping already exists; the hot path should be a cache/edge read, not a write to the mapping store.","tables":[{"name":"urls (shard p17)","cols":["short_key","long_url","user_id","created_at","expires_at","is_custom"],"rows":[{"c":["15ftgG","https://example.com/very/long/path?ref=x","42","2026-07-22 10:00:00","(null)","false"],"hi":1,"tag":"will read"}]},{"name":"Redis cache","cols":["cache_key","long_url","expires_at","ttl"],"rows":[{"c":["url:15ftgG","https://example.com/very/long/path?ref=x","(null)","hot"],"tag":"candidate"}]}]},narrate:"Redirects are the hot path: ~<code>116K</code> reads/s sustained, with 3-5x peaks and viral skew. The request is anonymous and must finish under 100ms p99, so every downstream choice tries to avoid origin work.",details:[
      ...
      {node:"lb",title:"Edge routes a cheap anonymous read",snap:{"cap":"No mutation: the gateway treats this as an anonymous redirect lookup and keeps create-only controls off the hot path. Edge cache can satisfy immutable links before origin.","tables":[{"name":"edge redirect cache","cols":["cache_key","long_url","redirect_code","state"],"rows":[{"c":["url:15ftgG","https://example.com/very/long/path?ref=x","302","miss → origin"],"hi":1,"tag":"route"}]}]},narrate:"The gateway can answer an edge-cached immutable redirect immediately, or route a miss to the shortener service. Unlike creates, this path avoids auth and heavy validation; rate limits focus on abusive high-miss clients.",details:[
      ...
      {node:"svc",title:"Service parses the key",snap:{"cap":"The service extracts a valid <code>short_key</code> and computes the same partition used at create time. Still read-only; any instance can serve the request because state lives in Redis/DB.","tables":[{"name":"urls (target shard p17)","cols":["short_key","long_url","user_id","created_at","expires_at","is_custom"],"rows":[{"c":["15ftgG","https://example.com/very/long/path?ref=x","42","2026-07-22 10:00:00","(null)","false"],"hi":1,"tag":"parsed key"}]}]},narrate:"The service extracts <code>short_key</code>, rejects invalid base62/alias shapes, and starts the read-through lookup. The service remains stateless: any instance can serve any key because routing is by <code>short_key</code> in cache/DB, not by session.",details:[
      ...
      {node:"cache",title:"Check Redis first",snap:{"cap":"Fast path: Redis has <code>url:15ftgG</code>, so the service can redirect without touching a DB replica. No mutation except optional TTL refresh; this protects the mapping store from 95%+ of reads.","tables":[{"name":"Redis cache","cols":["cache_key","long_url","expires_at","ttl"],"rows":[{"c":["url:15ftgG","https://example.com/very/long/path?ref=x","(null)","hot"],"hi":1,"tag":"hit"},{"c":["url:my-sale","https://shop.example.com/summer-sale","2026-08-01 00:00:00","warm"]}]}]},narrate:"A hot key should be served from Redis or the edge. With a ~95% hit ratio, the DB read load drops from ~116K/s to ~5.8K/s; for a viral key, local/edge caches prevent one Redis shard from becoming the bottleneck.",details:[
      ...
      {node:"replica",title:"On miss, read a replica",snap:{"cap":"Miss branch: Redis lacks the key, so one single-flight request reads a local DB replica. A brand-new-key miss falls back to the primary before returning 404 to avoid replication-lag false negatives.","tables":[{"name":"Redis cache","cols":["cache_key","long_url","expires_at","ttl"],"rows":[{"c":["url:15ftgG","<em>miss</em>","(null)","—"],"hi":1,"tag":"miss"}]},{"name":"urls (p17 read replica)","cols":["short_key","long_url","user_id","created_at","expires_at","is_custom"],"rows":[{"c":["15ftgG","https://example.com/very/long/path?ref=x","42","2026-07-22 10:00:00","(null)","false"],"hi":1,"tag":"read"}]}]},narrate:"Cache misses read from a DB replica to spare the write primary. Because mappings are immutable, a slightly stale replica is safe except for a brand-new key; on a miss for a fresh-looking key, fall back to the primary before returning 404.",details:[
      ...
      {node:"db",title:"Source of truth and expiry check",snap:{"cap":"The row is checked against <code>expires_at</code>; because it is <code>(null)</code>, redirect is allowed and the cache can be filled. Expired rows would return 410/404 and get short negative caching.","tables":[{"name":"urls (source of truth)","cols":["short_key","long_url","user_id","created_at","expires_at","is_custom"],"rows":[{"c":["15ftgG","https://example.com/very/long/path?ref=x","42","2026-07-22 10:00:00","(null)","false"],"hi":1,"tag":"not expired"},{"c":["my-sale","https://shop.example.com/summer-sale","7","2026-07-20 09:12:00","2026-08-01 00:00:00","true"]}]},{"name":"Redis cache","cols":["cache_key","long_url","expires_at","ttl"],"rows":[{"c":["url:15ftgG","https://example.com/very/long/path?ref=x","(null)","30d+jitter"],"hi":1,"tag":"filled on miss"}]}]},narrate:"If the replica has the row, the service checks <code>expires_at</code>. Expiry is a TTL/lazy-reclaim field in the store, not a scan; an expired row returns 410/404 and may be negative-cached briefly.",details:[
      ...
      {node:"analytics",title:"Emit click event asynchronously",snap:{"cap":"The redirect lookup is complete; now an analytics event is appended off-path. The user-facing redirect must not wait for this Kafka ack, so read correctness and click observability are deliberately decoupled.","tables":[{"name":"click_events","cols":["event_id","short_key","ts","country","referrer"],"rows":[{"c":["a1b2…","15ftgG","2026-07-22 10:05:11","US","https://twitter.com/"],"hi":1,"tag":"event built"}]},{"name":"urls (read row)","cols":["short_key","long_url","user_id","created_at","expires_at","is_custom"],"rows":[{"c":["15ftgG","https://example.com/very/long/path?ref=x","42","2026-07-22 10:00:00","(null)","false"],"tag":"read only"}]}],"queues":[{"name":"click-events","kind":"kafka","by":"key = short_key · 32 partitions","parts":[{"id":"P17","key":"← hash(15ftgG)","msgs":[{"v":"a1b2… 15ftgG US","hi":1,"tag":"appended @1042"}],"commit":1042,"end":1043},{"id":"P03","key":"other links","msgs":[{"v":"e9f0… my-sale IN"}],"commit":881,"end":882}]}]},narrate:"The redirect must not wait for analytics. The service creates a click event and hands it to Kafka with a bounded async producer; if the analytics path is slow, redirect latency wins and the event may be dropped/spilled depending on product accuracy needs.",details:[
      ...
      {node:"client",from:"svc",title:"Return 302 or 301",snap:{"cap":"Final read-only outcome: return <code>302</code> for analytics/control, or <code>301</code> only for permanent immutable links. The <code>urls</code> row is unchanged; analytics emission is already off-path.","tables":[{"name":"urls (shard p17)","cols":["short_key","long_url","user_id","created_at","expires_at","is_custom"],"rows":[{"c":["15ftgG","https://example.com/very/long/path?ref=x","42","2026-07-22 10:00:00","(null)","false"],"hi":1,"tag":"redirected"}]},{"name":"click_events","cols":["event_id","short_key","ts","country","referrer"],"rows":[{"c":["a1b2…","15ftgG","2026-07-22 10:05:11","US","https://twitter.com/"],"tag":"async"}]}]},narrate:"Finally the service returns the redirect. The default for a shortener with analytics, expiry, and revocation is <code>302 Found</code>; <code>301 Moved Permanently</code> is reserved for truly immutable links where maximum cacheability matters more than observability/control.",details:[
    ...
    {id:"analytics-e2e",name:"Aggregate clicks",summary:"Redirects enqueue click events off-path → Kafka partitions by <code>short_key</code> → consumers dedupe by <code>event_id</code> → shard hot counters/windowed aggregates → dashboards read eventually-consistent counts.",steps:[
      {node:"svc",title:"Redirect produces an event",snap:{"cap":"The service constructs a raw click event from the origin-visible redirect. It is not counted yet; the event becomes durable only when it is appended to the click stream.","tables":[{"name":"click_events","cols":["event_id","short_key","ts","country","referrer"],"rows":[{"c":["c3d4…","15ftgG","2026-07-22 10:05:12","IN","(direct)"],"hi":1,"tag":"created"},{"c":["a1b2…","15ftgG","2026-07-22 10:05:11","US","https://twitter.com/"]}]}]},narrate:"On each origin-visible redirect, the service constructs a click event from request metadata. This is deliberately after the mapping lookup but outside the user-visible critical path: analytics can lag or drop, redirects cannot.",details:[
      ...
      {node:"analytics",title:"Publish to the click stream",snap:{"cap":"Kafka append: key <code>15ftgG</code> routes to <strong>P17</strong>, record lands at offset 1043, and log-end advances to 1044. The producer is fire-and-forget from the redirect path, so delivery is at-least-once/best-effort depending on buffer policy.","tables":[{"name":"click_events","cols":["event_id","short_key","ts","country","referrer"],"rows":[{"c":["c3d4…","15ftgG","2026-07-22 10:05:12","IN","(direct)"],"hi":1,"tag":"produced"},{"c":["a1b2…","15ftgG","2026-07-22 10:05:11","US","https://twitter.com/"]}]}],"queues":[{"name":"click-events","kind":"kafka","by":"key = short_key · 32 partitions","parts":[{"id":"P17","key":"← hash(15ftgG)","msgs":[{"v":"a1b2… 15ftgG US"},{"v":"c3d4… 15ftgG IN","hi":1,"tag":"appended @1043"}],"commit":1042,"end":1044},{"id":"P03","key":"other short_keys","msgs":[{"v":"e9f0… my-sale US"}],"commit":881,"end":882}]}]},narrate:"Kafka/Kinesis absorbs spikes such as a marketing blast at ~500K clicks/s. Partition by <code>short_key</code> when per-link windows need local aggregation; for a single viral key, add a salt to avoid one partition taking 300K clicks/s.",details:[
        ...
        {k:"queue",label:"How Kafka adds the click",lang:"python",code:"producer.send('click-events', key=short_key, value=event, acks='1')\n# partition = hash('15ftgG') % 32 -> P17\n# append at offset 1043; log-end 1043 -> 1044; consumers track offsets"},
        {k:"queue",label:"Fire-and-forget trade-off",text:"The redirect path uses a bounded async producer: if Kafka slows, the buffer absorbs briefly, then drops or spills by product policy. That protects p99 redirect latency but means dashboard analytics are at-least-once / best-effort, not a synchronous correctness dependency."},
      ]},
      {node:"analytics",title:"Deduplicate before counting",snap:{"cap":"The analytics consumer group reads P17 at offset 1043 and inserts <code>event_id=c3d4…</code> into the dedup/raw event store. Commit advances only after the check-and-set succeeds, so a crash can re-read but not double-count.","tables":[{"name":"click_events","cols":["event_id","short_key","ts","country","referrer"],"rows":[{"c":["a1b2…","15ftgG","2026-07-22 10:05:11","US","https://twitter.com/"]},{"c":["c3d4…","15ftgG","2026-07-22 10:05:12","IN","(direct)"],"hi":1,"tag":"first seen"}]}],"queues":[{"name":"click-events","kind":"kafka","by":"consumer group 'click-aggregators'","parts":[{"id":"P17","key":"reading @1043","msgs":[{"v":"c3d4… 15ftgG IN","hi":1,"tag":"consuming @1043"}],"commit":1043,"end":1044},{"id":"P03","key":"other partition","msgs":[{"v":"e9f0… my-sale US"}],"commit":882,"end":882}]}]},narrate:"The pipeline is at-least-once: producer retries and consumer rebalances can replay an event. For dashboard counts that may be approximate, but for paid analytics or billing-grade reports, consumers dedupe on <code>event_id</code> within the aggregation window.",details:[
        ...
        {k:"queue",label:"How Kafka is consumed",lang:"python",code:"records = consumer.poll()          # group owns P17, reads offset 1043\n# insert event_id into dedup store, then update the window\nconsumer.commit()                 # after success: committed 1043 -> 1044\n# crash before commit => offset 1043 is re-read; event_id dedup makes it safe"},
      ]},
      {node:"analytics",title:"Aggregate into time windows",snap:{"cap":"After dedup, the same consumer group aggregates the event into the 10:05 one-minute window. The queue offset commits <strong>1043→1044</strong> only after the window state/rollup update is safe.","tables":[{"name":"click_events","cols":["event_id","short_key","ts","country","referrer"],"rows":[{"c":["c3d4…","15ftgG","2026-07-22 10:05:12","IN","(direct)"],"hi":1,"tag":"counted"},{"c":["a1b2…","15ftgG","2026-07-22 10:05:11","US","https://twitter.com/"]}]}],"queues":[{"name":"click-events","kind":"kafka","by":"consumer group 'click-aggregators'","parts":[{"id":"P17","key":"window 10:05 caught up","msgs":[{"v":"processed through c3d4…","hi":1,"tag":"commit @1044"}],"commit":1044,"end":1044}]}]},narrate:"Consumers do 1-minute tumbling windows keyed by <code>short_key</code>, country, and referrer. They write rollups, not one dashboard row per click; this turns a 500K/s firehose into a much smaller stream of aggregate updates.",details:[
        ...
        {k:"queue",label:"Ordering vs parallelism",text:"Keying by <code>short_key</code> keeps each link's click stream ordered on one partition, making per-link windows local. The cost is hot-key skew: a viral short key may need salting, and partition count caps the maximum number of active consumers."},
      ]},
      {node:"db",title:"Keep analytics off the mapping store",snap:{"cap":"No per-click mutation touches <code>urls</code>. The mapping row remains write-once while raw clicks and rollups live in the analytics path, avoiding a viral-key counter hotspot on the redirect DB.","tables":[{"name":"urls (mapping store)","cols":["short_key","long_url","user_id","created_at","expires_at","is_custom"],"rows":[{"c":["15ftgG","https://example.com/very/long/path?ref=x","42","2026-07-22 10:00:00","(null)","false"],"hi":1,"tag":"unchanged"}]},{"name":"click_events","cols":["event_id","short_key","ts","country","referrer"],"rows":[{"c":["c3d4…","15ftgG","2026-07-22 10:05:12","IN","(direct)"],"tag":"analytics"},{"c":["a1b2…","15ftgG","2026-07-22 10:05:11","US","https://twitter.com/"],"tag":"analytics"}]}]},narrate:"The <code>urls</code> table remains a write-once point-lookup mapping store. Analytics raw events and rollups belong in a log/OLAP path; mixing per-click writes into the mapping shard would turn the read-optimized redirect DB into a hot counter database.",details:[
      ...
      {node:"client",from:"analytics",title:"Dashboard reads eventual counts",snap:{"cap":"Dashboard data is eventual: it reflects Kafka consumer progress and deduped click events, not synchronous redirect writes. Counts may lag by seconds and 301-cached clicks may be absent by design.","tables":[{"name":"click_events (window 10:05)","cols":["event_id","short_key","ts","country","referrer"],"rows":[{"c":["a1b2…","15ftgG","2026-07-22 10:05:11","US","https://twitter.com/"]},{"c":["c3d4…","15ftgG","2026-07-22 10:05:12","IN","(direct)"],"hi":1,"tag":"included"}]},{"name":"urls (lookup metadata)","cols":["short_key","long_url","user_id","created_at","expires_at","is_custom"],"rows":[{"c":["15ftgG","https://example.com/very/long/path?ref=x","42","2026-07-22 10:00:00","(null)","false"],"tag":"not mutated"}]}]},narrate:"Users see near-real-time analytics with a clear consistency contract: counts may lag by consumer delay and can be approximate unless the product explicitly runs a slower deduplicated reconciliation path.",details:[
...
var scaling = {id:"scaling",name:"From one mapping table to viral redirects",kind:"scale",
  live:["client","lb","svc","db"],
  summary:"Start with the honest mapping table, then let the URL shortener numbers force the upgrades: unique-key generation, a read cache, replicas, and async analytics. Each stage adds exactly one box only after the previous design hits a real ceiling.",
  steps:[
    {node:"db",stage:"Stage 0 · Baseline",title:"One KV table — create inserts, redirect looks up by key",
      live:["client","lb","svc","db"],
