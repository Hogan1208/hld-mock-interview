...
window.DATA['feed'] = {
  ...
  dbDoc:{
    ...
    candidates:[
      {name:"PostgreSQL (relational)",ceiling:"~5-10K writes/s per primary; a few thousand point reads/s before vertical limits",nodes:"1 primary + read replicas; hundreds of TB and ~1M/s fan-out force hand-sharding across ~20+ nodes plus a stack of replicas",pick:false,note:"clean for the follow graph as relational rows, but ~1M/s fan-out writes and hundreds of TB blow past a single primary — you rebuild a distributed store by hand and never use the joins you paid for."},
      {name:"Cassandra / ScyllaDB (wide-column)",ceiling:"~10-50K writes/s per node, linear scale-out",nodes:"write-bound by fan-out: ~1M/s &divide; ~20K writes/node &approx; <strong>~50 nodes</strong> for the feed-list writes; storage adds more as data grows",pick:true,note:"chosen — a wide-row per user (<code>timeline:userId</code>) is exactly a partition, hash-partitioning spreads the fan-out write flood linearly, and tunable consistency fits eventual-read / read-your-writes."},
      {name:"Neo4j (native graph)",ceiling:"~10-20K traversal ops/s per node; not built for ~1M/s bulk-append",nodes:"single-digit nodes for the graph, but cannot absorb the fan-out write volume",pick:false,note:"great for deep multi-hop graph queries we do not need — the feed only needs one-hop adjacency (who follows whom), which a wide-column table does at far higher write throughput."},
    ],
    indexing:"Posts are point-lookup by <code>post_id</code> (hash-partitioned) for hydration, plus a per-author index on <code>(author_id, created_at)</code> so a celebrity read-time merge range-scans one author's recent posts cheaply. The follow graph is stored as <strong>bidirectional adjacency</strong> — both a followers list and a following list per user — because fan-out needs followers-of-author while the profile needs following-of-user; keeping both means every follow/unfollow is a <strong>double write</strong> (two rows), the deliberate write-cost trade for O(1) reads on the hot path. Feed lists (<code>timeline:userId</code>) are length-capped ordered lists, appended by fan-out, never scanned.",
    decision:"Pick a <strong>hash-partitioned wide-column store</strong> (Cassandra / ScyllaDB) for posts, follow edges, and feed lists. The deciding factor is the ~1M/s fan-out write flood plus hundreds of TB — that needs horizontal <em>write</em> scale, which comes from adding nodes, and a per-user wide row maps cleanly onto a partition. <strong>Not Postgres:</strong> 3,500 base posts/s fits one primary, but fan-out at ~1M/s and the storage force manual sharding + heavy replicas, rebuilding a distributed KV/wide-column store on an engine whose joins we never touch. <strong>Not a native graph DB:</strong> the feed only ever needs one-hop adjacency, not multi-hop traversal, and Neo4j cannot absorb the append volume — its strengths are wasted here. Strong consistency is only needed for read-your-own-writes, handled by a synchronous self-insert into the author's own list, not by adopting a relational DB wholesale.",
  },
  schema:{tables:[
  ...
  deepFlows:[
    {id:"post-fanout-e2e",name:"Post fan-out",summary:"**POST /post** → gateway write limits → durable <code>posts</code> insert by <code>post_id</code> → outbox to fan-out → normal authors push <code>post_id</code> into follower timelines by <code>user_id</code>; celebrities skip fan-out and are merged at read time.",steps:[
      {node:"client",title:"Client sends POST /post",snap:{cap:"Nothing durable yet &mdash; the client has only a stable idempotency key and media reference. The post will become <code>1487200000001</code> only after the feed service dedups and the DB commits.",tables:[{name:"posts (target shard 317)",cols:["post_id","author_id","text","media_ids","created_at"],rows:[{c:["1487200000001","42","just shipped the new release","[med_9f2a]","<em>not created</em>"],hi:1,tag:"pending write"}]}]},narrate:"The app sends the post body plus optional media references and a client-generated idempotency key. The key is what makes a subway retry safe: the user may lose the response, but the system must not create a duplicate visible post.",details:[
      ...
      {node:"lb",title:"Gateway authenticates and rate-limits",snap:{cap:"Still no mutation. The gateway admits a write for user 42 but applies stricter write limits because one accepted post can amplify into hundreds of timeline appends.",tables:[{name:"users (rate-limit context)",cols:["user_id","handle","follower_count","is_celebrity"],rows:[{c:["42","@ada","318","false"],hi:1,tag:"authenticated"},{c:["901","@popstar","51000000","true"]}]}]},narrate:"The gateway terminates TLS, authenticates the session, validates the payload size, and applies much stricter limits to writes than reads. A write is dangerous because it amplifies into downstream fan-out work; a read is mostly cache-absorbed.",details:[
      ...
      {node:"feed",title:"Feed service allocates the post id",snap:{cap:"The write is deduped and routed: <code>post_id 1487200000001</code> hashes to post shard 317. No canonical row exists until the next step commits.",tables:[{name:"posts (shard routing)",cols:["post_id","author_id","text","media_ids","created_at"],rows:[{c:["1487200000001","42","just shipped the new release","[med_9f2a]","allocated, not committed"],hi:1,tag:"allocated"},{c:["1487200000002","7","good morning","(null)","2026-07-22 10:01:12"]}]}]},narrate:"The feed service deduplicates the idempotency key, mints a globally unique Snowflake-style <code>post_id</code>, and prepares a canonical post row. For read-your-own-writes it will also synchronously add this id to the author's own timeline before returning.",details:[
      ...
      {node:"db",title:"Persist canonical post and outbox",snap:{cap:"The canonical post and transactional outbox intent commit together, then the outbox relay appends <code>evt_1487200000001</code> to Kafka partition <strong>P42</strong> keyed by <code>author_id=42</code>. The post is durable before any follower timeline is touched.",tables:[{name:"posts (post shard 317)",cols:["post_id","author_id","text","media_ids","created_at"],rows:[{c:["1487200000001","42","just shipped the new release","[med_9f2a]","2026-07-22 10:00:00"],hi:1,tag:"inserted"},{c:["1487200000002","7","good morning","(null)","2026-07-22 10:01:12"]}]},{name:"post_outbox (same commit)",cols:["event_id","post_id","author_id","created_at"],rows:[{c:["evt_1487200000001","1487200000001","42","2026-07-22 10:00:00"],hi:1,tag:"committed"}]}],queues:[{name:"post-fanout",kind:"kafka",by:"key = author_id · 96 partitions · RF=3",parts:[{id:"P42",key:"&larr; hash(42)",msgs:[{v:"evt_1487200000001 post 1487200000001 author 42",hi:1,tag:"appended @1205"}],commit:1205,end:1206},{id:"P17",key:"other authors",msgs:[{v:"evt_1487199999990 author 7"}],commit:822,end:823}]}]},narrate:"The post row is committed to the wide-column post store and an outbox event is committed in the same write transaction or batch. The API never acknowledges a post that exists only in memory or only in a queue.",details:[
        ...
        {k:"queue",label:"Transactional outbox hand-off",lang:"sql",code:"BEGIN;\nINSERT INTO posts (post_id, author_id, text, media_ids, created_at) VALUES (...);\nINSERT INTO post_outbox (event_id, post_id, author_id, created_at) VALUES (...);\nCOMMIT;\n-- relay publishes committed outbox rows to Kafka; no post can commit without its fan-out intent"},
        {k:"queue",label:"How Kafka adds the fan-out event",lang:"python",code:"producer.send('post-fanout', key=author_id, value=event, acks='all')\n# author_id=42 -> P42; broker appends evt_1487200000001 at offset 1205\n# log-end advances 1205 -> 1206; consumers track offsets, records are not removed"},
        {k:"queue",label:"Why key by author_id",text:"All events for one author land on one partition, preserving per-author post order for followers. More partitions give more parallelism across authors, while a celebrity's huge follower set is handled by hybrid read-time merge and chunked fan-out, not by randomizing this event."},
      ]},
      {node:"fanout",title:"Fan-out consumes the outbox event",snap:{cap:"Fan-out group <code>home-timeline-writers</code> owns P42 and reads offset 1205. The offset is deliberately uncommitted until follower timelines are updated, so a crash replays the event.",tables:[{name:"users",cols:["user_id","handle","follower_count","is_celebrity"],rows:[{c:["42","@ada","318","false"],hi:1,tag:"normal author"},{c:["901","@popstar","51000000","true"]}]},{name:"follows (followers of 42)",cols:["follower_id","followee_id","created_at"],rows:[{c:["7","42","2026-06-10 12:15:00"],hi:1,tag:"fan-out target"},{c:["42","901","2026-06-01 08:00:00"]}]}],queues:[{name:"post-fanout",kind:"kafka",by:"consumer group home-timeline-writers",parts:[{id:"P42",key:"reading author 42",msgs:[{v:"evt_1487200000001 post 1487200000001",hi:1,tag:"reading @1205"}],commit:1205,end:1206},{id:"P17",key:"other authors",msgs:[{v:"evt_1487199999990 author 7"}],commit:823,end:823}]}]},narrate:"Fan-out workers read the committed event and decide whether this author is a normal account or a celebrity. Normal accounts fan out on write; celebrities above the threshold do not, because a 50M-follower post would otherwise become 50M immediate writes.",details:[
        ...
        {k:"queue",label:"How Kafka is polled",lang:"python",code:"records = consumer.poll()  # group owns P42 and reads offset 1205\n# look up followers, then write timeline ids idempotently\n# do NOT commit yet if cache writes fail; crash before commit re-reads offset 1205"},
        {k:"queue",label:"Commit only after timelines update",lang:"python",code:"ZADD timeline:7 score post_id   # idempotent sorted-set add\nZADD timeline:42 score post_id  # read-your-own-writes self insert\nconsumer.commit()              # committed offset 1205 -> 1206 only after success"},
        {k:"queue",label:"At-least-once plus idempotent append",text:"Kafka may redeliver the same event after a crash; <code>ZADD</code> by <code>post_id</code> updates the same member rather than duplicating it. Correctness comes from idempotent timeline writes, not from exactly-once delivery."},
      ]},
      {node:"cache",title:"Append ids to follower timelines",snap:{cap:"Timeline id-lists, not post bodies, are updated for normal-account followers and the author self-list. The write is idempotent: replaying offset 1205 re-adds the same post id and trims to the same bounded list.",tables:[{name:"timelines (feed cache / Redis)",cols:["user_id","post_ids","updated_at"],rows:[{c:["7","[1487200000001]","2026-07-22 10:00:00"],hi:1,tag:"follower append"},{c:["42","[1487200000001,1487200000002]","2026-07-22 10:00:00"],hi:1,tag:"self insert"}]},{name:"posts (canonical unchanged)",cols:["post_id","author_id","text","media_ids","created_at"],rows:[{c:["1487200000001","42","just shipped the new release","[med_9f2a]","2026-07-22 10:00:00"],hi:1,tag:"referenced only"}]}],queues:[{name:"post-fanout",kind:"kafka",by:"consumer group home-timeline-writers",parts:[{id:"P42",key:"caught up",msgs:[{v:"(offset 1205 processed)"}],commit:1206,end:1206}]}]},narrate:"For each non-celebrity follower, fan-out writes the <code>post_id</code> into that user's bounded Redis sorted set and trims it to the hot window. The shard key is the destination <code>user_id</code> because reads fetch one user's home timeline.",details:[
      ...
      {node:"client",title:"Return after durable post + self insert",snap:{cap:"The response can be sent because the canonical post is durable and the author timeline includes the id. Follower delivery is eventually consistent; celebrity posts would be pulled at read time instead of pushed to every follower.",tables:[{name:"posts",cols:["post_id","author_id","text","media_ids","created_at"],rows:[{c:["1487200000001","42","just shipped the new release","[med_9f2a]","2026-07-22 10:00:00"],hi:1,tag:"durable"}]},{name:"timelines",cols:["user_id","post_ids","updated_at"],rows:[{c:["42","[1487200000001,1487200000002]","2026-07-22 10:00:00"],hi:1,tag:"read-your-writes"},{c:["7","[1487200000001]","2026-07-22 10:00:00"]}]}]},narrate:"The client gets a response after the canonical post is durable and the author's own timeline contains the id. Followers see it seconds later as async fan-out drains; celebrity followers see it when their read path merges recent celebrity posts.",details:[
    ...
    {id:"timeline-read-e2e",name:"Read timeline",summary:"**GET /timeline** → route to the user's home region → read <code>timeline:userId</code> by <code>user_id</code> → merge recent celebrity posts by <code>(author_id, created_at)</code> → hydrate <code>posts</code> by <code>post_id</code> → return a cursor under the 200ms p99 target.",steps:[
      {node:"client",title:"Client asks for a page",snap:{cap:"Read-only: no mutation. The cursor seeks into user 42's precomputed id-list so new head posts do not shift the next page boundary.",tables:[{name:"timelines (reader cache)",cols:["user_id","post_ids","updated_at"],rows:[{c:["42","[1487200000002,1487200000001]","2026-07-22 10:01:12"],hi:1,tag:"will read"}]}]},narrate:"Opening the app is the hot path: ~6B reads/day, ~70K/s average and up to ~300K/s peak. The request carries an opaque cursor, not an offset, so new posts at the head do not shift every page boundary.",details:[
      ...
      {node:"lb",title:"Gateway routes the read cheaply",snap:{cap:"Read-only: no mutation. The gateway routes user 42 to the home-region feed fleet and protects the hot timeline path from optional embellishments.",tables:[{name:"users (routing context)",cols:["user_id","handle","follower_count","is_celebrity"],rows:[{c:["42","@ada","318","false"],hi:1,tag:"home region"}]}]},narrate:"The gateway authenticates and routes to the nearest healthy feed-service instance in the user's home region. Personalized timelines are not CDN-cacheable, so the edge's job is fast routing and shedding optional work, not serving the feed body itself.",details:[
      ...
      {node:"feed",title:"Feed service starts a bounded build",snap:{cap:"Read-only: no mutation. The build plan is bounded to one timeline list read, a small celebrity merge, and a page-sized hydration set &mdash; never a live scan of all followees.",tables:[{name:"follows (reader adjacency)",cols:["follower_id","followee_id","created_at"],rows:[{c:["42","901","2026-06-01 08:00:00"],hi:1,tag:"celebrity merge"},{c:["42","7","2026-06-10 12:15:00"],hi:1,tag:"precomputed"}]},{name:"timelines",cols:["user_id","post_ids","updated_at"],rows:[{c:["42","[1487200000002,1487200000001]","2026-07-22 10:01:12"],hi:1,tag:"bounded list"}]}]},narrate:"The stateless feed service fans out only to bounded, known stores: one timeline range read, a small celebrity merge, and parallel hydration of the page ids. It never queries all 300 followees' posts on the hot path.",details:[
      ...
      {node:"cache",title:"Read the precomputed ids",snap:{cap:"Read-only: no mutation. Redis returns ids from <code>timeline:42</code>; bodies stay canonical in <code>posts</code>, keeping the hot read a tiny single-shard range read.",tables:[{name:"timelines (feed cache shard hash(42))",cols:["user_id","post_ids","updated_at"],rows:[{c:["42","[1487200000002,1487200000001]","2026-07-22 10:01:12"],hi:1,tag:"range read"},{c:["7","[1487200000001]","2026-07-22 10:00:00"]}]}]},narrate:"For normal followees, the feed already exists as an ordered set of post ids. The key is the reader's <code>user_id</code>, so the read touches one Redis shard and returns ids, not 1KB post bodies.",details:[
      ...
      {node:"db",title:"Pull recent celebrity posts",snap:{cap:"Read-only: no mutation. User 42 follows celebrity 901, so the read path range-scans only recent posts by 901 and merges those ids with the cached normal-author list.",tables:[{name:"follows",cols:["follower_id","followee_id","created_at"],rows:[{c:["42","901","2026-06-01 08:00:00"],hi:1,tag:"followed celeb"},{c:["42","7","2026-06-10 12:15:00"]}]},{name:"users",cols:["user_id","handle","follower_count","is_celebrity"],rows:[{c:["901","@popstar","51000000","true"],hi:1,tag:"pull model"}]},{name:"posts (author_id,created_at index)",cols:["post_id","author_id","text","media_ids","created_at"],rows:[{c:["1487200000003","901","tour dates announced","[med_1b3c,med_1b3d]","2026-07-22 10:02:30"],hi:1,tag:"merge candidate"}]}]},narrate:"For followed celebrities, there was no write-time fan-out. The feed service reads the user's followed celebrity ids and range-scans only their recent posts using the per-author <code>(author_id, created_at)</code> index, then merges those candidates with the cached list.",details:[
      ...
      {node:"db",title:"Hydrate post ids from canonical storage",snap:{cap:"Read-only: no mutation. The merged id list is hydrated with parallel point reads by <code>post_id</code>; slow or deleted rows can be dropped to preserve p99.",tables:[{name:"posts (scatter-gather by post_id)",cols:["post_id","author_id","text","media_ids","created_at"],rows:[{c:["1487200000003","901","tour dates announced","[med_1b3c,med_1b3d]","2026-07-22 10:02:30"],hi:1,tag:"hydrated"},{c:["1487200000002","7","good morning","(null)","2026-07-22 10:01:12"],hi:1,tag:"hydrated"},{c:["1487200000001","42","just shipped the new release","[med_9f2a]","2026-07-22 10:00:00"],hi:1,tag:"hydrated"}]}]},narrate:"The merged list contains ids only. Hydration scatter-gathers those <code>post_id</code>s to the hash-partitioned post store and fetches author metadata in parallel, with a tight deadline so one cold shard does not stall the whole feed.",details:[
      ...
      {node:"media",title:"Return media references, not bytes",snap:{cap:"Read-only: no mutation. The feed response carries <code>media_ids</code> copied from canonical post rows; image and video bytes stay on the CDN path.",tables:[{name:"posts (response projection)",cols:["post_id","author_id","text","media_ids","created_at"],rows:[{c:["1487200000001","42","just shipped the new release","[med_9f2a]","2026-07-22 10:00:00"],hi:1,tag:"media ref"},{c:["1487200000003","901","tour dates announced","[med_1b3c,med_1b3d]","2026-07-22 10:02:30"],hi:1,tag:"media ref"}]}]},narrate:"The feed response includes <code>media_ids</code> and CDN URLs or rendition metadata; the heavy image/video bytes are fetched by the client from the CDN in parallel with text rendering.",details:[
      ...
      {node:"client",title:"Client renders and keeps the cursor",snap:{cap:"Read-only: no mutation. The client stores the opaque cursor while the server-side timeline and canonical posts remain unchanged for the next page seek.",tables:[{name:"timelines",cols:["user_id","post_ids","updated_at"],rows:[{c:["42","[1487200000002,1487200000001]","2026-07-22 10:01:12"],hi:1,tag:"cursor after 1487200000001"}]},{name:"posts",cols:["post_id","author_id","text","media_ids","created_at"],rows:[{c:["1487200000001","42","just shipped the new release","[med_9f2a]","2026-07-22 10:00:00"],hi:1,tag:"rendered"}]}]},narrate:"The client renders whatever arrived before the deadline and stores the cursor for the next page. If a few hydrated posts were deleted or slow, the service can drop them and backfill later; a fast slightly-imperfect feed beats a perfect feed that misses p99.",details:[
  ...
  q:{
    ...
    db:[
      {l:"hard",tag:"concept",q:"Choosing the datastore(s): candidates, throughput ceilings, indexing, decision.",turns:[
        {who:"intv",text:"Before schema — choose the actual engine. First characterize the read and write load this store must carry, then pick and defend it against the alternatives with real per-node throughput numbers. This is the decision I want documented."},
        {who:"cand",text:"Let me pin the load first, because it drives the choice. <strong>Writes:</strong> ~3,500 posts/s, plus follow-edge writes well under that — and critically the scary ~1M/s of feed-list writes do <em>not</em> land here, they live in the feed cache. So the durable store sees only low-thousands of writes/s. <strong>Reads:</strong> two shapes. Hydration is a high-volume <strong>point lookup by post id</strong> (the read the cache misses fall through to) — call it low hundreds of K/s at peak after the cache absorbs most. Plus two graph reads: <em>who I follow</em> (build candidate authors) and <em>who follows me</em> (drive delivery), billions/day. <strong>Scale:</strong> billions of posts (~1.7PB replicated over 5y) and ~180B follow edges. So I need cheap point reads at massive volume, modest writes, adjacency reads both directions, and horizontal scale to petabytes. That profile says <strong>split by workload</strong>: a post store and a graph store, chosen separately."},
        {who:"intv",text:"Good. Give me the candidate engines for the post store, their rough per-node throughput ceilings, and the node/shard math that follows."},
        {who:"cand",text:"Three candidates. <strong>(1) Single-node relational (Postgres-style):</strong> per-node ceiling roughly ~5-10K writes/s and ~20-40K indexed point reads/s, but it falls apart past billions of rows / a few TB without hand-rolled sharding. <strong>(2) Wide-column / KV (Cassandra/DynamoDB-style):</strong> ~10-50K writes/s per node, native point-lookup by key, and linear scale-out by adding nodes. <strong>(3) Native graph DB (Neo4j-style):</strong> excellent traversals but weak at billions of edges under high write and hard to shard horizontally. For posts the access pattern is pure point-lookup by id, no joins, so the math is storage-bound not throughput-bound.<span class='eg'>Posts ~1.7PB replicated / ~2TB usable per node → ~850 nodes. Spread 3,500 writes/s over ~850 nodes ≈ 4 writes/s/node; ~200K point reads/s over ~850 ≈ 240 reads/s/node — both trivially under a wide-column node's 10-50K ceiling.</span>So capacity, not throughput, sets the node count, and a wide-column store scales to that node count linearly. A single relational cluster would need heavy manual sharding just to hold the rows — reinventing what the wide-column store gives natively."},
        {who:"intv",text:"Now the graph store. Why not fold it into that same post store, and how do you index the two access patterns — posts by author/time, and the follow adjacency in both directions — including the write cost of those indexes?"},
        {who:"cand",text:"Different access pattern, so different store: adjacency-list and range reads, not point lookups. I use a <strong>TAO-style association store</strong> — a graph-cache layer over sharded persistent storage. <strong>Indexing the two patterns:</strong> for the follow graph I store each edge <strong>bidirectionally</strong> — a <code>following</code> index keyed by follower_id and a <code>followers</code> index keyed by followee_id — so both <em>who I follow</em> and <em>who follows me</em> are a single keyed range scan. Write cost: every follow is <strong>two index writes</strong>, reconciled async, and a divergent edge just delays one feed insert, not correctness. For <strong>posts by author/time</strong> I keep a secondary index table keyed <code>(author_id, created_at desc)</code> so a per-author recent-posts list (profile view, and the celebrity read-time merge) is one range scan — write cost is <strong>one extra index write per post</strong> on top of the canonical post row.<span class='eg'>Graph: 180B edges x ~50B ≈ 9TB raw, x3 replication ≈ 27TB → ~14 nodes on storage alone — tiny next to posts. But adjacency reads at hundreds of K/s against a ~30-50K assoc-reads/s/node ceiling push it to dozens of nodes, and the TAO cache layer absorbs the bulk so the persistent tier stays small.</span>"},
        {who:"intv",text:"A single hot user with 50M followers — how does that index survive on one shard, and what about split-brain on the two-write follow?"},
        {who:"cand",text:"A 50M-entry followers list can't sit on one key or one shard — that's a hot partition. I <strong>sub-partition</strong> the followers of a hot user across many partitions (hash of follower_id), so reads and writes of that list parallelize N-ways instead of pinning one node; normal users keep a single-partition edge list. The two-write follow is eventually consistent — I write both directions and a background repair reconciles divergence — so a momentarily half-written edge only delays a feed insert. This is exactly why a native graph DB loses here: it nails traversals but won't shard a 180B-edge, hot-skewed graph under this write+read rate."},
        {who:"intv",text:"Commit to the decision — which engine for which store, and one line on why not each alternative."},
        {who:"cand",text:"<strong>Decision.</strong> <strong>Post store: wide-column / KV (Cassandra/DynamoDB-style)</strong>, hash-sharded by post_id, with the <code>(author_id, created_at)</code> secondary index — chosen because posts are storage-bound point-lookups that scale linearly to ~850 nodes / petabytes with huge write headroom. <strong>Graph store: TAO-style association layer</strong> over sharded storage with bidirectional adjacency indexes — chosen because the workload is cheap neighbor-list reads billions of times a day with a caching layer in front. <strong>Why not the others:</strong> single-node relational can't hold billions of posts or 180B edges without reinventing sharding and its per-node ceiling is too low; a native graph DB can't horizontally shard the hot-skewed graph at this write+read rate; and folding graph into the post KV forces adjacency/range access onto a point-lookup engine and fights the celebrity hot-partition. Two stores, each matched to its access pattern, is the documented call."},
      ],resources:[
        {title:"Facebook TAO — the social graph store (USENIX)",url:"https://www.usenix.org/conference/atc13/technical-sessions/presentation/bronson"},
        {title:"Consistent hashing",url:"https://en.wikipedia.org/wiki/Consistent_hashing"},
      ]},
  ...
  mockTest:[
    {q:"Explain fan-out on write vs fan-out on read, and why this design uses a hybrid split by author popularity.",a:"Fan-out on write pushes a new post id into every follower's pre-built feed list at post time, so reads are a cheap list fetch — great for the 20:1 read:write ratio. Fan-out on read merges followees' recent posts at query time, cheap to write but expensive on every read. Pure write-fan-out explodes on a celebrity (one post becomes tens of millions of list writes); pure read-fan-out makes every feed load a large merge. The hybrid fans out normal accounts on write and skips fan-out for celebrities above a follower threshold, merging their recent posts in at read time — so neither worst case can occur."},
    {q:"A user follows 300 accounts. Walk through what happens on GET /timeline and why feed lists store post ids rather than post bodies.",a:"Range-read the top page of post ids from the user's feed cache list (a Redis sorted set), merge in the handful of followed celebrities at read time, then hydrate those ids with a parallel scatter-gather multi-get against the post store for bodies, authors, and media refs, and return the page. Lists hold ids not bodies because a popular post lands in millions of feed lists — inlining the ~1KB body would duplicate it millions of times and make an edit or delete rewrite millions of copies, whereas an 8-byte id keeps one canonical copy and lets millions of lists fit in memory."},
    {q:"Size the feed cache: how much RAM and how many nodes for the active-user timelines?",a:"Store per active user a sorted set of post ids trimmed to the top few hundred. At ~800 ids x 8 bytes ≈ 6.4KB, times ~3x for sorted-set overhead ≈ 20KB per timeline. Across ~300M active users that is roughly 6TB; at ~50GB usable per node ≈ 120 nodes, doubled to ~240 with a replica per shard plus headroom. Storing ids not bodies is what keeps this in the terabytes rather than hundreds of terabytes."},
    {q:"Choose the datastore(s) for posts and the follow graph and justify the split.",a:"Split by workload. Posts go in a wide-column / KV store (Cassandra/DynamoDB-style), hash-sharded by post id — point-lookup access, huge write headroom, and it scales linearly to the ~850 nodes needed for ~1.7PB. The follow graph goes in a TAO-style association store over sharded storage, holding edges bidirectionally (following by follower_id, followers by followee_id) so both adjacency directions are one keyed range scan. A single-node relational engine cannot hold billions of rows or 180B edges without reinventing sharding, and a native graph DB will not horizontally shard the hot-skewed graph at this read+write rate."},
    {q:"A celebrity with 50M followers posts. What breaks under naive fan-out and how do you handle both the write and the follower-list read?",a:"Naive fan-out turns one post into 50M feed-list writes, saturating the fan-out tier and cache and delaying delivery for everyone. Celebrities above the threshold are exempted from fan-out entirely — their posts sit in a per-author recent-posts list and are merged in at read time, so the 50M writes become zero plus a tiny bounded merge only active followers pay. For the upper-mid tier that still fans out, the follower list is sub-partitioned across many nodes and read as a parallel paged scan streamed in chunks to workers, so no single shard is pinned."},
    {q:"A feed cache node restarts empty and millions of timelines cold-miss. How do you avoid showing blank feeds?",a:"Never return empty. Run the cache as a replicated cluster so a node restart fails over to a replica that still holds the lists. If a list is genuinely gone, rebuild it on demand from the durable source — who the user follows plus recent posts — since every feed list is a recomputable view, not authoritative data. Rebuild lazily only when a user actually loads (spreading work across real traffic rather than a stampede), protect the store with request coalescing and per-shard rate limits, and shard the cache widely so any one node's loss has a small blast radius. A cache loss is a latency event, never data loss."},
    {q:"How does the design guarantee read-your-own-writes when a post is fanned out asynchronously?",a:"On a successful post, synchronously insert the post id into the author's own feed list before returning 200, so the self-view is guaranteed even before async fan-out reaches other followers. The client also holds the post from its optimistic-UI ack and can prepend it locally, and for a short window after a write the author's reads are pinned to the freshest source rather than a lagging replica. Followers can be eventually consistent — seconds is imperceptible — but the author must never doubt their own post landed."},
