...
window.DATA['ticketmaster'] = {
  ...
  deepFlows:[
    {id:"reserve-checkout-e2e",name:"Reserve & checkout",summary:"**POST /reserve** → admitted token + idempotency → **atomic hold on <code>(event_id, seat_id)</code>** → 10-minute TTL → idempotent payment → **held→sold** quorum commit. The DB, not Redis or the client, decides who owns the seat.",steps:[
      {node:"client",title:"Client chooses exact seats",snap:{cap:"Read-only choice from a stale map: <code>s-103</code> still looks available, but this is only a hint. The authoritative state changes only when the inventory DB accepts the conditional hold.",tables:[{name:"seats (evt-501 snapshot)",cols:["seat_id","event_id","section","row","num","status","hold_expires_at","held_by"],rows:[{c:["s-101","evt-501","Floor A","1","14","sold","(null)","(null)"]},{c:["s-102","evt-501","Floor A","1","15","held","2026-07-22 10:03:00","user-9001"]},{c:["s-103","evt-501","Floor A","1","16","available","(null)","(null)"],hi:1,tag:"selected"}]}]},narrate:"The seat map is only a **display hint**. The client sends the exact <code>event_id</code> and <code>seat_ids</code>, a reused idempotency key, and the admitted waiting-room token; the reserve response is the first authoritative fact.",details:[
      ...
      {node:"gw",title:"Gateway admits only paced buyers",snap:{cap:"The gateway accepts only a signed admitted token. Demand can be 1M users, but only the active pool reaches booking; everyone else remains a cheap waiting-room row.",tables:[{name:"waiting_room (virtual · Redis sorted set)",cols:["user_id","position","token","admitted?"],rows:[{c:["user-8120","481999","qtok_wait_evt501_481999","no"]},{c:["user-9001","482000","qtok_admitted_evt501_000482","yes"],hi:1,tag:"admitted"},{c:["user-7777","482001","qtok_wait_evt501_482001","no"]}]}]},narrate:"The gateway validates auth, per-user limits, and the waiting-room token before the request can touch booking. This is what keeps the raw **~1M users in one second** from becoming ~1M DB writes.",details:[
      ...
      {node:"booking",title:"Booking routes the seat write",snap:{cap:"No mutation yet: booking deterministically routes <code>evt-501:s-103</code> to inventory partition P17. Sub-partitioning by seat keeps one hot event from pinning a single shard.",tables:[{name:"inventory routing",cols:["event_id","seat_id","partition"],rows:[{c:["evt-501","s-103","P17"],hi:1,tag:"target"},{c:["evt-501","s-101","P03"]},{c:["evt-501","s-102","P11"]}]},{name:"seats (partition P17)",cols:["seat_id","event_id","status","hold_expires_at","held_by"],rows:[{c:["s-103","evt-501","available","(null)","(null)"],hi:1,tag:"will write"}]}]},narrate:"Booking is stateless, so it recomputes the inventory partition from the seat identity. The natural top-level key is <code>event_id</code>, but one hot event would pin one shard, so the actual partition includes the seat id or seat block.",details:[
      ...
      {node:"db",title:"Atomically place the hold",snap:{cap:"The conditional update is the concurrency point: <code>s-103</code> moves <strong>available &rarr; held</strong> only if the row is still available or the prior hold expired. A pending order is created in the same transaction.",tables:[{name:"seats (partition P17 leader)",cols:["seat_id","event_id","section","row","num","status","hold_expires_at","held_by"],rows:[{c:["s-103","evt-501","Floor A","1","16","held","2026-07-22 10:12:55","user-9001"],hi:1,tag:"held"}]},{name:"orders",cols:["order_id","user_id","event_id","seat_ids","status","payment_id","created_at"],rows:[{c:["ord-8003","user-9001","evt-501","[s-103]","pending","(null)","2026-07-22 10:02:55"],hi:1,tag:"inserted"}]}]},narrate:"The reserve is one short ACID transaction on the inventory leader. The conditional update treats an expired hold as claimable, so the hold is self-expiring even if booking or the reaper dies.",details:[
      ...
      {node:"db",title:"Commit with strong consistency",snap:{cap:"The held seat and pending order are acked only after leader + follower durability. A crash after this point can lose neither the hold nor the order intent.",tables:[{name:"inventory partition P17 · replica set",cols:["replica","role","has s-103 hold?"],rows:[{c:["az-a","leader","yes · WAL fsync"],hi:1,tag:"durable"},{c:["az-b","follower","yes · quorum ack"],hi:1,tag:"durable"},{c:["az-c","follower","catching up"]}]},{name:"seats (committed)",cols:["seat_id","event_id","status","hold_expires_at","held_by"],rows:[{c:["s-103","evt-501","held","2026-07-22 10:12:55","user-9001"],hi:1,tag:"quorum"}]}]},narrate:"Inventory is the sacred write path: every reserve and confirmed order commits on the leader and is synchronously replicated to a quorum before the caller sees success. Reads can be stale; inventory writes cannot.",details:[
      ...
      {node:"booking",title:"Return the hold and countdown",snap:{cap:"The client receives durable state: order <code>ord-8003</code> is pending and seat <code>s-103</code> is held until 10:12:55. No DB lock is held during checkout.",tables:[{name:"orders",cols:["order_id","user_id","event_id","seat_ids","status","payment_id","created_at"],rows:[{c:["ord-8003","user-9001","evt-501","[s-103]","pending","(null)","2026-07-22 10:02:55"],hi:1,tag:"returned"}]},{name:"seats",cols:["seat_id","event_id","status","hold_expires_at","held_by"],rows:[{c:["s-103","evt-501","held","2026-07-22 10:12:55","user-9001"],hi:1,tag:"countdown"}]}]},narrate:"Booking returns a pending order and the server-generated expiry timestamp. The lock is not a long DB lock; it is durable seat state with a TTL field, so the user can spend minutes in checkout without holding a row lock.",details:[
      ...
      {node:"payment",title:"Charge with payment idempotency",snap:{cap:"Payment is external to the seat transaction. The idempotency key <code>pay-ord-8003</code> maps to one authorization, so retrying after a timeout cannot create a second charge.",tables:[{name:"payments",cols:["payment_id","order_id","amount","status"],rows:[{c:["pay-7002","ord-8003","180.00","authorized"],hi:1,tag:"idempotent auth"}]},{name:"orders",cols:["order_id","status","payment_id"],rows:[{c:["ord-8003","pending","(null)"],hi:1,tag:"not sold yet"}]},{name:"seats",cols:["seat_id","status","hold_expires_at","held_by"],rows:[{c:["s-103","held","2026-07-22 10:12:55","user-9001"]}]}]},narrate:"Payment runs **outside** the inventory transaction. Booking sends the order id as the payment idempotency key so a timeout or retry cannot double-charge the buyer.",details:[
      ...
      {node:"db",title:"Confirm only if the hold is still ours",snap:{cap:"The confirmation is conditional: sell <code>s-103</code> only while it is still <code>held</code> by <code>user-9001</code> and unexpired. If that predicate fails, booking compensates the payment instead of force-selling.",tables:[{name:"seats",cols:["seat_id","event_id","status","hold_expires_at","held_by"],rows:[{c:["s-103","evt-501","sold","(null)","(null)"],hi:1,tag:"held→sold"}]},{name:"orders",cols:["order_id","user_id","event_id","seat_ids","status","payment_id","created_at"],rows:[{c:["ord-8003","user-9001","evt-501","[s-103]","confirmed","pay-7002","2026-07-22 10:02:55"],hi:1,tag:"confirmed"}]},{name:"payments",cols:["payment_id","order_id","amount","status"],rows:[{c:["pay-7002","ord-8003","180.00","captured"],hi:1,tag:"captured"}]}]},narrate:"After payment succeeds, booking performs a second short transaction. It re-checks that <code>s-103</code> is still <code>held</code> by this user and not expired, then atomically marks the seat sold and confirms the order.",details:[
      ...
      {node:"client",from:"db",title:"Client receives the ticket",snap:{cap:"Final state is recoverable from the database: one sold seat, one confirmed order, one captured payment. Retries now read the same terminal order instead of selling again.",tables:[{name:"seats",cols:["seat_id","event_id","section","row","num","status","hold_expires_at","held_by"],rows:[{c:["s-103","evt-501","Floor A","1","16","sold","(null)","(null)"],hi:1,tag:"ticketed"}]},{name:"orders",cols:["order_id","user_id","event_id","seat_ids","status","payment_id","created_at"],rows:[{c:["ord-8003","user-9001","evt-501","[s-103]","confirmed","pay-7002","2026-07-22 10:02:55"],hi:1}]},{name:"payments",cols:["payment_id","order_id","amount","status"],rows:[{c:["pay-7002","ord-8003","180.00","captured"],hi:1}]}]},narrate:"Only after the confirmed order is durably committed does the user see a ticket. The system has charged at most once, sold the seat at most once, and can recover the result from <code>orders</code>, <code>payments</code>, and <code>seats</code> after any crash.",details:[
    ...
    {id:"browse-onsale-e2e",name:"Browse at on-sale",summary:"**1M viewers** hit the event page → edge/waiting room absorbs the herd → search and seat-map reads stay eventually consistent → cache fan-out serves ~500K/s → only reserve writes use the strongly-consistent inventory leader.",steps:[
      {node:"client",title:"A million clients open the event",snap:{cap:"Read-only herd: event metadata, queue position, and a stale seat map are safe to cache. No inventory row mutates just because 1M clients load the page.",tables:[{name:"events",cols:["event_id","name","venue_id","starts_at","on_sale_at"],rows:[{c:["evt-501","Taylor Swift - Eras Tour","ven-11","2026-09-01 20:00:00","2026-07-22 10:00:00"],hi:1,tag:"read"}]},{name:"venues",cols:["venue_id","name","seat_map_id"],rows:[{c:["ven-11","Gillette Stadium","map-900"],hi:1,tag:"read"}]},{name:"waiting_room (virtual)",cols:["user_id","position","token","admitted?"],rows:[{c:["user-9001","482000","qtok_wait_evt501_482000","no"],hi:1,tag:"in line"}]}]},narrate:"At 10:00:00, users mostly read: event details, queue position, and seat availability. These reads are **not** allowed to compete with the inventory write path that prevents double-selling.",details:[
      ...
      {node:"gw",title:"Edge separates reads from scarce writes",snap:{cap:"The edge answers most read traffic and blocks unadmitted writes. Inventory remains untouched until a valid admitted token appears on <code>POST /reserve</code>.",tables:[{name:"edge route table",cols:["request","served_by","mutation?"],rows:[{c:["GET /seatmap evt-501","CDN / cache","no"],hi:1,tag:"cached read"},{c:["GET /waiting-room position","edge cached watermark","no"]},{c:["POST /reserve","booking only with ADMITTED token","yes"]}]},{name:"seats (inventory unchanged)",cols:["seat_id","event_id","status","hold_expires_at","held_by"],rows:[{c:["s-103","evt-501","available","(null)","(null)"],hi:1,tag:"no mutation"}]}]},narrate:"The gateway/CDN answers cacheable browse and queue-position reads at the edge. Only admitted, authenticated reserve attempts flow toward booking.",details:[
      ...
      {node:"queue",title:"Waiting room makes demand bounded",snap:{cap:"The virtual queue advances by position, not by a broker message. The dispatcher marks the next batch admitted and mints a short-lived token; downstream sees only the paced active pool.",tables:[{name:"waiting_room (virtual · Redis sorted set)",cols:["user_id","position","token","admitted?"],rows:[{c:["user-8999","481999","qtok_admitted_evt501_000481","yes"]},{c:["user-9001","482000","qtok_admitted_evt501_000482","yes"],hi:1,tag:"admitted"},{c:["user-9002","482001","qtok_wait_evt501_482001","no"]}]},{name:"admission watermark",cols:["event_id","admitted_through","active_pool"],rows:[{c:["evt-501","482000","~5K"],hi:1,tag:"advanced"}]}]},narrate:"Unadmitted users do not call booking. They poll a cheap status value while the dispatcher emits admitted tokens at the backend's safe service rate.",details:[
        ...
        {k:"queue",label:"Virtual queue admission",lang:"redis",code:"ZADD waiting_room:evt-501 1784704200000 user-9001\n# position = rank in sorted set; refresh reads cached admitted_through\nZRANK waiting_room:evt-501 user-9001  # 482000\nZPOPMIN waiting_room:evt-501 500      # dispatcher advances a bounded batch"},
        {k:"queue",label:"Bounded concurrency token",lang:"text",code:"active_pool_target = 5000\nif active_pool < target: mark next positions ADMITTED and sign token\nGateway accepts POST /reserve only with admitted=true and unexpired token.\nNo Kafka/SQS semantics here: it is position state + signed admission."},
      ]},
      {node:"search",title:"Search is a stale read model",snap:{cap:"Search reads its own eventually-consistent index. It may show an availability badge that lags inventory, but this is a read-only discovery result and cannot mutate seats.",tables:[{name:"search index events-v7",cols:["event_id","name","venue_id","starts_at","availability_badge"],rows:[{c:["evt-501","Taylor Swift - Eras Tour","ven-11","2026-09-01 20:00:00","available (stale-ok)"],hi:1,tag:"read model"}]},{name:"events (source row)",cols:["event_id","name","venue_id","starts_at","on_sale_at"],rows:[{c:["evt-501","Taylor Swift - Eras Tour","ven-11","2026-09-01 20:00:00","2026-07-22 10:00:00"]}]}]},narrate:"Search/browse uses its own replicated index over event metadata. It is fed asynchronously from catalog/inventory changes, so a stale 'available' badge can happen, but it cannot sell a seat.",details:[
      ...
      {node:"booking",title:"Booking asks for the seat map",snap:{cap:"Seat-map fetch is a derived read, not an inventory decision. The hot event is replicated across cache keys; <code>s-103</code> can be highlighted as the row returned to the client.",tables:[{name:"seat-map cache (replica #7)",cols:["seat_id","event_id","section","row","num","status","hold_expires_at","held_by"],rows:[{c:["s-101","evt-501","Floor A","1","14","sold","(null)","(null)"]},{c:["s-102","evt-501","Floor A","1","15","held","2026-07-22 10:03:00","user-9001"]},{c:["s-103","evt-501","Floor A","1","16","available","(null)","(null)"],hi:1,tag:"cache hit"}]}]},narrate:"The event page needs a 100K-seat availability snapshot. Booking reads the derived map from cache, not the inventory leader. On a miss, it uses coalescing so only one fill query runs.",details:[
      ...
      {node:"cache",title:"Cache stays eventually consistent",snap:{cap:"After a successful reserve elsewhere, cache replicas learn <code>s-103</code> is held. This is a derived update; if one replica lags, reserve still re-checks the DB.",tables:[{name:"seat-map cache (replica #7)",cols:["seat_id","event_id","section","row","num","status","hold_expires_at","held_by"],rows:[{c:["s-103","evt-501","Floor A","1","16","held","2026-07-22 10:12:55","user-9001"],hi:1,tag:"event applied"}]},{name:"seats (source of truth)",cols:["seat_id","event_id","status","hold_expires_at","held_by"],rows:[{c:["s-103","evt-501","held","2026-07-22 10:12:55","user-9001"]}]}]},narrate:"Every reserve/release publishes a seat-state update, and the cache converges within about a second. It is deliberately biased to mark seats taken quickly and freed seats slightly later.",details:[
      ...
      {node:"db",title:"Misses avoid the write primary",snap:{cap:"A cache miss is coalesced and filled from a follower/read snapshot. The write leader is protected for conditional <code>held</code> and <code>sold</code> transitions.",tables:[{name:"seats (read replica fill)",cols:["seat_id","event_id","section","row","num","status","hold_expires_at","held_by"],rows:[{c:["s-101","evt-501","Floor A","1","14","sold","(null)","(null)"]},{c:["s-102","evt-501","Floor A","1","15","held","2026-07-22 10:03:00","user-9001"]},{c:["s-103","evt-501","Floor A","1","16","available","(null)","(null)"],hi:1,tag:"stale-ok read"}]},{name:"inventory write leader",cols:["partition","reserved_for","read_misses"],rows:[{c:["P17","reserve/confirm writes","blocked by single-flight"],hi:1,tag:"protected"}]}]},narrate:"If the cache misses or goes cold, the fill path reads from replicas/read-optimized snapshots and uses request coalescing. The write leader is reserved for holds and confirmations.",details:[
      ...
      {node:"client",from:"cache",title:"Client renders a live-ish map",snap:{cap:"The UI shows <code>as_of</code> and TTL so users understand the map is live-ish. Clicking <code>s-103</code> switches to the strongly-consistent reserve flow where the DB decides.",tables:[{name:"seat-map response",cols:["seat_id","event_id","section","row","num","status","hold_expires_at","held_by"],rows:[{c:["s-101","evt-501","Floor A","1","14","sold","(null)","(null)"]},{c:["s-102","evt-501","Floor A","1","15","held","2026-07-22 10:03:00","user-9001"]},{c:["s-103","evt-501","Floor A","1","16","available","(null)","(null)"],hi:1,tag:"display hint"}]},{name:"waiting_room (virtual)",cols:["user_id","position","token","admitted?"],rows:[{c:["user-9001","482000","qtok_admitted_evt501_000482","yes"],hi:1,tag:"can reserve"}]}]},narrate:"The user sees a near-real-time map and queue position. When they click a seat, the flow switches from cheap, stale reads to the strongly-consistent reserve path above.",details:[
  ...
  steps:[
    {node:"db",stage:"Stage 0 · Baseline",title:"One inventory DB — conditional reserve is the truth",
      ...
      details:[
        ...
        {k:"pain",label:"What breaks — a concrete case",text:"At **Eclipse Dome**, Ticketmaster opens sales for **The Comets Final Tour**: **100K seats**, and **1M fans** hammer Buy at 10:00:00. The baseline DB is logically correct: if two fans tap section 114 row C seat 12, exactly one conditional update returns row_count 1. But if all 1M requests reach the same inventory tier at once, the database spends seconds rejecting losers, connection pools fill, and real winners time out before seeing their hold. Correctness exists; controlled load does not."},
        {k:"fix",label:"The fix — walk the same case, single authority first",text:"Keep the first invariant simple: only the inventory row can grant a hold, and every retry carries an idempotency key. For a paced beta where **&le;5K reserve attempts/s** reach the DB, one conditional write per seat is enough and losers get clean 409s. The design is intentionally not ready for raw Eclipse Dome demand; each later stage exists to keep traffic at the DB's safe ceiling without ever moving seat truth elsewhere."},
        {k:"host",label:"Load & capacity — what runs it",text:"**PostgreSQL or CockroachDB inventory**, 1 write primary plus 2 synchronous replicas for the launch slice, table keyed `(event_id, seat_id)`. Seat data is tiny: **100K seats × ~1 KB ≈ 100 MB/event**, even **10K events ≈ 1 TB** raw. The hard ceiling is write contention: a single well-tuned primary can do **~5−10K conditional writes/s**; at Eclipse's raw 1M/s, it is overloaded by **100×**, so admission control is mandatory."},
      ],
      snap:{title:"Load & capacity — Stage 0",cap:"The invariant is correct, but payment and on-sale pressure have not arrived yet.",
        tables:[{name:"signals",cols:["signal","value","verdict"],rows:[
          {c:["Inventory store","1 primary + 2 sync replicas · PostgreSQL/CockroachDB","N+1 for node loss"],hi:1},
          {c:["Seat data","100K seats × ~1 KB ≈ 100 MB/event","tiny"]},
          {c:["Conditional writes","~5−10K/s ceiling","ok if paced"],hi:1},
          {c:["Eclipse raw demand","~1M Buy clicks in 1 s","100× too high"],hi:1,tag:"risk"},
          {c:["Browse load","~10K req/s steady","ok early"]},
        ]}] }},
    {node:"payment",stage:"Stage 1 · Two-phase checkout",title:"Payment cannot hold a row lock &rarr; hold, then confirm",
      ...
      details:[
        {k:"pain",label:"What breaks — a concrete case",text:"Eclipse fan Asha wins seat 114-C-12 at 10:00:03, then her bank opens 3-D Secure and she takes **22 s** to approve. If booking keeps the seat row locked while charging, every retry, alternate buyer, and admin reaper check for that seat queues behind Asha's bank call. Hot-seat throughput collapses to **1 ÷ 22 s ≈ 0.045 reserves/s**. The DB is not double-selling; it is holding scarce inventory hostage to a human payment step."},
        {k:"fix",label:"The fix — walk the same case, reserve, pay, confirm",text:"Reserve flips `available&rarr;held` and writes `hold_expires_at = now + 10 min` in **~5 ms**, then releases the row. Payment runs outside the lock with an idempotency key. When Asha approves 22 s later, confirm flips `held&rarr;sold` in another short transaction. The hot row was locked for **5 ms, not 22 s**, so theoretical row throughput moves from **0.045/s** to **~200/s**, and abandoned carts self-expire."},
        {k:"host",label:"Load & capacity — what runs it",text:"**Booking service** is stateless, 3−5 nodes at ~1K ops/s each, sized for the admitted pool; **inventory DB** remains the same 1 primary + 2 sync replicas. Holds and orders share one ACID transaction on `(event_id, seat_id)` plus an idempotency table keyed by client token. A small reaper worker scans `hold_expires_at` via index; even **5K active holds** over 10 minutes is only hundreds of expiries/s, far below the write ceiling."},
      ...
      snap:{title:"Load & capacity — Stage 1",cap:"Checkout time no longer determines inventory lock time.",
        tables:[{name:"signals",cols:["signal","before","after"],rows:[
          {c:["Inventory store","1 primary + 2 sync replicas","unchanged · N+1"],hi:1},
          {c:["Lock hold time","~1−30 s payment path","~5 ms reserve txn"],hi:1,tag:"fixed"},
          {c:["Hot-seat throughput","~0.045/s at 22 s payment","~200/s at 5 ms"],hi:1},
          {c:["Active holds","unbounded stuck carts","~5K admitted · 10 min TTL"]},
          {c:["Retry safety","duplicate charge risk","idempotency key"]},
        ]}] }},
    {node:"queue",stage:"Stage 2 · Waiting room",title:"Million-user on-sale &rarr; admit at backend capacity",
      ...
      details:[
        {k:"pain",label:"What breaks — a concrete case",text:"At 10:00:00, Eclipse receives **1M Buy clicks** and every browser retries after **3 s** if it sees a spinner. Without a waiting room, gateway sees hundreds of thousands of reserve attempts, booking opens too many payment sessions, and the DB's **5−10K writes/s** ceiling is buried under rejects and retries. Fans who would have won seats time out, then retry and make the spike worse. Raw internet demand becomes a denial-of-service against the correctness path."},
        {k:"fix",label:"The fix — walk the same case, virtual waiting room",text:"The queue accepts the million arrivals cheaply, assigns positions, and releases signed admission tokens at **~5K active checkouts** and **~2−3K short booking ops/s**. Everyone else polls queue status from a cheap key-value store or CDN response. Eclipse still sells the same 100K seats, but the DB never sees more work than it can serialize; users wait in line instead of all competing inside the transaction layer."},
        {k:"host",label:"Load & capacity — what runs it",text:"**Waiting room service** backed by Redis Cluster for counters/tokens plus Kafka for append-only arrival audit. Use **64 Kafka partitions by event_id bucket**: if 1M arrivals land in 10 s, that's **100K joins/s ÷ 64 ≈ 1.6K joins/s/partition**, trivial for Kafka; partitions are for parallel consumers and preserving per-bucket order. Redis uses 3 masters + 3 replicas per region; tokens are signed and expire, so queue state loss cannot sell a seat."},
      ...
      snap:{title:"Load & capacity — Stage 2",cap:"Demand becomes a controlled admission stream instead of a backend stampede.",
        tables:[{name:"signals",cols:["signal","value","verdict"],rows:[
          {c:["Queue store","Redis Cluster 3 masters + 3 replicas · signed tokens","N+1"],hi:1,tag:"fixed"},
          {c:["Arrival log","Kafka 64 partitions by event bucket · RF=3","ordered + parallel"],hi:1},
          {c:["On-sale demand","~1M users in the same second","queued"]},
          {c:["Partition math","100K joins/s ÷ 64 ≈ 1.6K/s each","headroom"]},
          {c:["Active pool","~5K admitted · ~2−3K ops/s","bounded backend"],hi:1},
        ]}] }},
    {node:"cache",stage:"Stage 3 · Seat-map cache",title:"Read firehose threatens writes &rarr; cache availability",
      ...
      details:[
        {k:"pain",label:"What breaks — a concrete case",text:"While only 5K Eclipse fans are admitted, almost everyone watches the map. **1M clients polling every 2 s ≈ 500K seat-map reads/s**, and the hot event alone is **~450K/s**. If each repaint reads `seats where event_id=E`, read CPU and buffer cache evict the rows the reserve path must update. The DB starts missing its 5 ms conditional-write target; stale green seats become slow 500s instead of clean 409s."},
        {k:"fix",label:"The fix — walk the same case, cache the read model",text:"Serve the Eclipse map from CDN plus Redis. The CDN handles public static sections and 1−2 s TTL map tiles; Redis stores per-section availability bitsets updated after hold, release, and sale events. With a **95% edge hit ratio**, origin drops from **500K/s** to **25K/s**; Redis absorbs that, and the inventory primary keeps its write budget. A stale green seat still goes through reserve and returns 409 if already held."},
        {k:"host",label:"Load & capacity — what runs it",text:"**Redis Cluster seat-map cache**, 6 masters + 6 replicas for the hot region, sharded by `(event_id, section_id)`. Eclipse has roughly 100 sections, so **25K origin reads/s ÷ 6 ≈ 4.2K reads/s/master**, tiny versus **~100K ops/s/master**. CDN edge takes the remaining **~475K/s**. Inventory DB replicas are not the display tier; they serve cold cache fills and admin reads only."},
      ...
      snap:{title:"Load & capacity — Stage 3",cap:"The huge read path is now isolated from the scarce write path.",
        tables:[{name:"signals",cols:["signal","before cache","after cache"],rows:[
          {c:["Seat-map store","inventory DB reads","CDN + Redis Cluster 6 masters + 6 replicas"],hi:1,tag:"fixed"},
          {c:["Seat-map reads","~500K/s to origin risk","~95% at edge · ~25K/s Redis"]},
          {c:["Per-Redis-master load","not applicable","25K ÷ 6 ≈ 4.2K/s · ample headroom"],hi:1},
    ...
    {node:"search",stage:"Stage 4 · Search",title:"Discovery queries grow &rarr; split a search index",
      ...
      details:[
        {k:"pain",label:"What breaks — a concrete case",text:"The Comets announce a second Eclipse date and social traffic sends **50K discovery queries/s** for artist, city, price, and accessible-seat facets while the first date is still selling. If these faceted text queries run on inventory, Elasticsearch-like CPU work competes with cache fills and booking metadata. A search for \"Comets nearby\" should never slow seat 114-C-12's conditional update."},
        {k:"fix",label:"The fix — walk the same case, replicated inverted index",text:"Catalog changes, event metadata, and coarse availability hints flow by CDC into Elasticsearch/OpenSearch. Search can lag **a few seconds**: it may still show Eclipse as available, but clicking lands on the seat map and reserve path, where Redis and the DB correct truth before money moves. Direct event links continue working if search is degraded, because buying does not depend on the index."},
        {k:"host",label:"Load & capacity — what runs it",text:"**OpenSearch/Elasticsearch**, 12 primary shards with 1 replica each across 6 data nodes. At **50K queries/s**, replicas give **24 shard copies**, so average fan-out capacity is roughly **50K ÷ 24 ≈ 2.1K query slices/s per shard copy**; the catalog is only a few GB, so it fits in page cache. Use 3 dedicated masters for quorum; 1 replica is enough because search is rebuildable from CDC and not the source of truth."},
      ...
      snap:{title:"Load & capacity — Stage 4 (full design)",cap:"Discovery, display, admission, payment, and inventory now scale on separate axes.",
        tables:[{name:"signals",cols:["concern","mechanism","result"],rows:[
          {c:["Search index","OpenSearch · 12 primaries + 1 replica · 6 data nodes","N+1 rebuildable"],hi:1,tag:"fixed"},
          {c:["Discovery QPS","~50K/s","~2.1K query slices/s per shard copy"],hi:1},
          {c:["Index size","few GB catalog + hints","fits memory"]},
          {c:["Freshness","seconds of lag","safe for browse"]},
          {c:["Buying invariant","conditional DB write","no double-sell"],hi:1},
        ]}] }},
  ]};
