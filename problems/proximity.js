...
window.DATA['proximity'] = {
  ...
  deepFlows:[
    {id:"nearby-search-e2e",name:"Find nearby drivers",summary:"**GET /nearby** → gateway → match computes an **H3 ring** → geo-index reads exact <code>cell_id</code> buckets → fetch <code>driver_locations</code> + <code>drivers</code> → haversine filter/rank → short-TTL cache → return candidates under the <200ms p99 budget.",steps:[
      {node:"client",title:"Rider asks for nearby supply",snap:{cap:"Read-only start: rider <code>55012</code> asks around downtown SF. No table mutates yet; the live rows below are the data this query will eventually narrow.",tables:[{name:"driver_locations (SF live shard · no mutation)",cols:["driver_id","h3_cell","lat","lng","ts"],rows:[{c:["8821","8a2830828047fff","37.7749","-122.4194","10:05:03"],hi:1,tag:"near query point"},{c:["8830","8a2830828047fff","37.7752","-122.4188","10:05:02"]},{c:["8822","8a283082807ffff","37.7811","-122.4102","10:05:01"]}]}]},narrate:"The rider app sends the query point and radius. The example uses the same SF coordinates and H3 cells already present in the schema; for a Yelp-style places endpoint the object type changes, but the proximity path is identical.",details:[
      ...
      {node:"gw",title:"Gateway authenticates and routes the read",snap:{cap:"The gateway makes no data change: it authenticates rider <code>55012</code> and routes this read to the SF match shard while keeping driver writes on ingest.",tables:[{name:"rides (dispatch store · read not created yet)",cols:["ride_id","rider_id","driver_id","state","created_at"],rows:[{c:["b12c…","55019","(null)","offered","10:05:06"]},{c:["nearby-55012","55012","(null)","not a ride yet","—"],hi:1,tag:"read-only query"}]}]},narrate:"The API gateway validates the rider token, rate-limits abusive clients, and sends only latency-sensitive nearby reads to the match service. Driver write traffic stays isolated on the location-ingest path.",details:[
      ...
      {node:"match",title:"Compute the H3 cells to query",snap:{cap:"Match computes <code>origin=8a2830828047fff</code> and includes neighbor <code>8a283082807ffff</code> for the 2km radius. No mutation; this is the exact cell route for the index reads.",tables:[{name:"geo_index routing (SF region · computed cells)",cols:["cell_id","driver_ids","driver_count","updated_at"],rows:[{c:["8a2830828047fff","{8821, 8830, 8845}","3","10:05:03"],hi:1,tag:"origin"},{c:["8a283082807ffff","{8822}","1","10:05:01"],hi:1,tag:"neighbor"}]}]},narrate:"The match service maps the query point to its H3 cell and chooses the neighbor ring covering the radius. Dense cities use finer cells (H3 res ~10, ~65m); sparse areas can use coarser cells (res ~7, ~1km).",details:[
      ...
      {node:"match",title:"Check the hot-cell read cache first",snap:{cap:"Hot-cell cache hits for this exact cell ring: read-only and <strong>no mutation</strong>. The cached id set is a speed layer; live rows are still hydrated and TTL-filtered next.",tables:[{name:"nearby candidate cache (match shard sf-downtown)",cols:["cache_key","driver_ids","age_ms","ttl_ms"],rows:[{c:["nearby:8a2830828047fff:r2000:available:v1","{8821, 8830, 8845, 8822}","850","1000"],hi:1,tag:"hit"}]},{name:"geo_index (source rows · no mutation)",cols:["cell_id","driver_ids","driver_count","updated_at"],rows:[{c:["8a2830828047fff","{8821, 8830, 8845}","3","10:05:03"]},{c:["8a283082807ffff","{8822}","1","10:05:01"]}]}]},narrate:"Many riders in the same downtown block ask almost the same question during surge. The match tier can reuse a candidate-id set for a sub-second TTL, then still compute exact distance against each rider's actual lat/lng.",details:[
      ...
      {node:"index",title:"Read the geo-index buckets",snap:{cap:"If the cache misses, the geo-index does point reads by <code>cell_id</code> on the SF cell shard. Highlighted rows are read, not changed; the union is <code>{8821,8830,8845,8822}</code>.",tables:[{name:"geo_index (SF cell shard · read-only)",cols:["cell_id","driver_ids","driver_count","updated_at"],rows:[{c:["8a2830828047fff","{8821, 8830, 8845}","3","10:05:03"],hi:1,tag:"SMEMBERS"},{c:["8a283082807ffff","{8822}","1","10:05:01"],hi:1,tag:"SMEMBERS"}]}]},narrate:"On a cache miss, the match service asks the geo-index for each cell's driver id set. The index key is <code>cell_id</code>, so every lookup is a point read against a small Redis set, not a distance scan.",details:[
      ...
      {node:"db",title:"Hydrate live positions and availability",snap:{cap:"Hydration reads current positions and availability with <strong>no mutation</strong>. 8845 was in the bucket but has no fresh location row; 8822 is present but busy, so both are filtered before ranking.",tables:[{name:"driver_locations (SF live shard · no mutation)",cols:["driver_id","h3_cell","lat","lng","ts"],rows:[{c:["8821","8a2830828047fff","37.7749","-122.4194","10:05:03"],hi:1,tag:"fresh"},{c:["8830","8a2830828047fff","37.7752","-122.4188","10:05:02"],hi:1,tag:"fresh"},{c:["8822","8a283082807ffff","37.7811","-122.4102","10:05:01"],hi:1,tag:"fresh"},{c:["8845","8a2830828047fff","—","—","expired"],gone:1,tag:"TTL"}]},{name:"drivers (status shard · no mutation)",cols:["driver_id","status","last_lat","last_lng","updated_at"],rows:[{c:["8821","available","37.7749","-122.4194","10:05:03"],hi:1},{c:["8830","available","37.7752","-122.4188","10:05:02"],hi:1},{c:["8822","busy","37.7811","-122.4102","10:05:01"],tag:"filtered"}]}]},narrate:"Candidate ids are not enough; the match service fetches the current coordinates from <code>driver_locations</code> and joins availability from <code>drivers</code>. TTL and status filtering remove stale/offline entries before ranking.",details:[
      ...
      {node:"match",title:"Exact filter, rank, and cap",snap:{cap:"Match computes exact distance on hydrated available rows only. No table changes: 8821 and 8830 remain candidates; 8822 is busy and 8845 expired, so the capped result has two drivers.",tables:[{name:"driver_locations (rank input · no mutation)",cols:["driver_id","h3_cell","lat","lng","ts"],rows:[{c:["8821","8a2830828047fff","37.7749","-122.4194","10:05:03"],hi:1,tag:"rank #1"},{c:["8830","8a2830828047fff","37.7752","-122.4188","10:05:02"],hi:1,tag:"rank #2"},{c:["8822","8a283082807ffff","37.7811","-122.4102","10:05:01"],tag:"busy drop"}]}]},narrate:"The index returns a superset. Match computes haversine distance from the rider point, drops candidates outside 2km, ranks by distance/ETA, and caps the result so one crowded cell cannot explode per-query CPU.",details:[
      ...
      {node:"client",title:"Return ranked nearby candidates",snap:{cap:"Response returns the ranked, fresh subset with cache age metadata. Still read-only: dispatch may later create or claim a <code>rides</code> row, but nearby search itself does not.",tables:[{name:"driver_locations (returned candidates · no mutation)",cols:["driver_id","h3_cell","lat","lng","ts"],rows:[{c:["8821","8a2830828047fff","37.7749","-122.4194","10:05:03"],hi:1,tag:"eta 55s"},{c:["8830","8a2830828047fff","37.7752","-122.4188","10:05:02"],hi:1,tag:"eta 65s"}]},{name:"rides (unchanged)",cols:["ride_id","rider_id","driver_id","state","created_at"],rows:[{c:["b12c…","55019","(null)","offered","10:05:06"]}]}]},narrate:"The client receives a bounded list with freshness metadata so downstream dispatch can decide whether to offer immediately or refresh before assignment.",details:[
    ...
    {id:"location-index-write-e2e",name:"Update a driver's location",summary:"Driver frame → gateway rate limit → ingest batches latest-per-driver → compute <code>h3_cell</code> → upsert <code>driver_locations</code> with TTL → conditional move in <code>geo_index</code> only on cell change → replicate in-memory shards and fork durable history off-path.",steps:[
      {node:"client",title:"Driver streams a new GPS sample",snap:{cap:"Driver <code>8821</code> emits seq <code>918273</code> from the neighbor cell toward the origin cell. Nothing is stored yet; current state still shows the old bucket membership.",tables:[{name:"driver_locations (before update · driver shard 21)",cols:["driver_id","h3_cell","lat","lng","ts"],rows:[{c:["8821","8a283082807ffff","37.7811","-122.4102","10:05:03"],hi:1,tag:"old position"}]},{name:"geo_index (before update · SF cell shard)",cols:["cell_id","driver_ids","driver_count","updated_at"],rows:[{c:["8a283082807ffff","{8821, 8822}","2","10:05:03"],hi:1,tag:"old cell"},{c:["8a2830828047fff","{8830, 8845}","2","10:05:02"],tag:"target cell"}]}]},narrate:"A moving driver sends tiny fire-and-forget frames every ~4s. The client does not wait for durable storage because the next sample supersedes this one almost immediately.",details:[
      ...
      {node:"gw",title:"Gateway protects the ingest path",snap:{cap:"The edge accepts only the newest allowed frame for driver 8821 and routes by <code>hash(driver_id)</code> to ingest partition 21. No durable or index mutation yet.",tables:[{name:"ingest admission (partition 21 · transient)",cols:["driver_id","seq","status"],rows:[{c:["8821","918273","accepted newest"],hi:1,tag:"token ok"},{c:["8821","918271","dropped older"],gone:1,tag:"latest-wins"}]},{name:"driver_locations (unchanged before ingest flush)",cols:["driver_id","h3_cell","lat","lng","ts"],rows:[{c:["8821","8a283082807ffff","37.7811","-122.4102","10:05:03"]}]}]},narrate:"The gateway authenticates the driver connection and applies a per-driver token bucket. If a buggy app sends 50 updates/s, the edge keeps the newest frame and drops intermediates.",details:[
      ...
      {node:"location",title:"Batch and collapse to latest per driver",snap:{cap:"A 100ms ingest batch collapses multiple samples for 8821 to seq <code>918273</code>. This table is transient state only; downstream will write exactly one latest position.",tables:[{name:"ingest batch buffer (partition 21 · transient)",cols:["driver_id","seq","lat","lng","ts"],rows:[{c:["8821","918271","37.7750","-122.4190","10:05:06.800"],gone:1,tag:"superseded"},{c:["8821","918273","37.7752","-122.4188","10:05:07"],hi:1,tag:"kept"},{c:["8830","551002","37.7752","-122.4188","10:05:07"],tag:"other driver"}]}]},narrate:"Location ingest batches over ~50-100ms, normalizes the point, and retains only the highest-sequence sample for each driver in the batch. It also forks a durable event to Kafka for analytics/history outside the hot path.",details:[
      ...
      {node:"location",title:"Compute the target H3 cell",snap:{cap:"Ingest computes the new cell <code>8a2830828047fff</code> and compares it to old <code>8a283082807ffff</code>; this flags a real bucket move.",tables:[{name:"driver_locations (lookup old cell · driver shard 21)",cols:["driver_id","h3_cell","lat","lng","ts"],rows:[{c:["8821","8a283082807ffff","37.7811","-122.4102","10:05:03"],hi:1,tag:"old cell"}]},{name:"geo_index route plan",cols:["cell_id","driver_ids","driver_count","updated_at"],rows:[{c:["8a283082807ffff","{8821, 8822}","2","10:05:03"],tag:"remove from"},{c:["8a2830828047fff","{8830, 8845}","2","10:05:02"],hi:1,tag:"add to"}]}]},narrate:"The ingest service computes the cell before writing. If the cell did not change, it skips the expensive index move and only refreshes the live position TTL.",details:[
      ...
      {node:"db",title:"Upsert the latest live position",snap:{cap:"Latest-position upsert lands on the driver-id shard: driver 8821 now points at <code>8a2830828047fff</code> with TTL refreshed to ~15s. The geo-index has not moved yet in this step.",tables:[{name:"driver_locations (driver shard 21)",cols:["driver_id","h3_cell","lat","lng","ts"],rows:[{c:["8821","8a2830828047fff","37.7752","-122.4188","10:05:07"],hi:1,tag:"upserted"},{c:["8830","8a2830828047fff","37.7752","-122.4188","10:05:02"]}]},{name:"drivers (status shard)",cols:["driver_id","status","last_lat","last_lng","updated_at"],rows:[{c:["8821","available","37.7752","-122.4188","10:05:07"],hi:1,tag:"status fresh"}]}]},narrate:"The hot location store overwrites one row keyed by <code>driver_id</code>. The entry has a ~15s TTL so an offline driver automatically disappears after 2-3 missed reports.",details:[
      ...
      {node:"index",title:"Move between geo-index buckets if needed",snap:{cap:"The geo-index move is the key state change: remove 8821 from old cell <code>8a283082807ffff</code> and add it to new cell <code>8a2830828047fff</code>. Counts stay consistent with the live-position upsert.",tables:[{name:"geo_index (SF cell shard · bucket move)",cols:["cell_id","driver_ids","driver_count","updated_at"],rows:[{c:["8a283082807ffff","{8822}","1","10:05:07"],hi:1,tag:"removed 8821"},{c:["8a283082807ffff","{8821, 8822}","2","10:05:03"],gone:1,tag:"old bucket"},{c:["8a2830828047fff","{8821, 8830, 8845}","3","10:05:07"],hi:1,tag:"added 8821"}]},{name:"driver_locations (driver shard 21)",cols:["driver_id","h3_cell","lat","lng","ts"],rows:[{c:["8821","8a2830828047fff","37.7752","-122.4188","10:05:07"],hi:1}]}]},narrate:"If the computed H3 cell changed, ingest removes the driver id from the old cell's set and adds it to the new cell's set. This is derived state, but the move should be atomic enough that a driver is not missing from both cells.",details:[
      ...
      {node:"index",title:"Replicate, expire, and rebuild if necessary",snap:{cap:"Primary applies the move and an AZ replica catches up. TTLs make stale rows disappear automatically; if both copies are lost, the next driver reports rebuild these same bucket rows.",tables:[{name:"geo_index (primary · SF cell shard)",cols:["cell_id","driver_ids","driver_count","updated_at"],rows:[{c:["8a2830828047fff","{8821, 8830, 8845}","3","10:05:07"],hi:1,tag:"primary"},{c:["8a283082807ffff","{8822}","1","10:05:07"]}]},{name:"geo_index (AZ replica · SF cell shard)",cols:["cell_id","driver_ids","driver_count","updated_at"],rows:[{c:["8a2830828047fff","{8821, 8830, 8845}","3","10:05:07"],hi:1,tag:"replicated"},{c:["8a283082807ffff","{8822}","1","10:05:07"]}]}]},narrate:"The hot stores run primary + AZ replica groups because availability matters during surge, but the data is still rebuildable from current locations or the next driver reports.",details:[
      ...
      {node:"location",title:"Acknowledge only receipt, not durability",snap:{cap:"Ack means ingest accepted the newest sample, not that a durable queue committed it. Query-visible state is now consistent: live row and bucket both place 8821 in <code>8a2830828047fff</code> until superseded or TTL expiry.",tables:[{name:"driver_locations (query-visible final)",cols:["driver_id","h3_cell","lat","lng","ts"],rows:[{c:["8821","8a2830828047fff","37.7752","-122.4188","10:05:07"],hi:1,tag:"fresh until 10:05:22"},{c:["8830","8a2830828047fff","37.7752","-122.4188","10:05:02"]}]},{name:"geo_index (query-visible final)",cols:["cell_id","driver_ids","driver_count","updated_at"],rows:[{c:["8a2830828047fff","{8821, 8830, 8845}","3","10:05:07"],hi:1},{c:["8a283082807ffff","{8822}","1","10:05:07"]}]}]},narrate:"The driver can receive a lightweight ack (or no per-frame ack) once ingest accepted the newest sample. The correctness contract is freshness, not historical completeness.",details:[
...
var scaling={id:"scaling",name:"From bounding-box scan to real-time dispatch",kind:"scale",
  ...
  steps:[
    {node:"db",stage:"Stage 0 · Baseline",title:"Bounding-box query over current positions",
      ...
      details:[
        {k:"pain",label:"What breaks — a concrete case",text:"At **22:05**, the **Pier 48 concert** ends and 18K riders open the app around the same blocks in San Francisco. The MVP runs a bounding-box query over the current driver table, then haversine-filters the result. With the real platform load of **~1M active drivers** and **~80K nearby queries/s peak**, that is potentially **80K scans/s × 1M rows = 80B row checks/s** before ranking. Even though the working set is only **~150 MB**, CPU melts, p99 blows past **200 ms**, and the nearest cars disappear behind a global scan."},
        {k:"fix",label:"The fix — walk the same case, prove the simple shape",text:"For a small launch, keep the simple current-position table: one latest row per driver, TTL **~15 s**, and exact distance filtering after the bounding box. Pier 48 works at pilot scale, for example **20K drivers** and **100 queries/s**, because the scan is a few million row checks/s and fits in RAM. The same flow is intentionally drawn first so the correctness is clear; Stage 1 replaces only the access path when the concert-scale numbers arrive."},
        {k:"host",label:"Load &amp; capacity — what runs it",text:"Baseline can be **PostgreSQL/PostGIS 1 primary + 2 read replicas** for the pilot, with `driver_locations(driver_id, lat, lng, ts)` and a TTL-style freshness filter. Math says why it cannot be the final hot store: durable spatial updates are **~5–10K writes/s/node**, while production drivers send **250K writes/s**; queries at **80K/s** cannot scan 1M rows. The 3-node Postgres group is N+1 for correctness testing, not production scale. The later Redis GEO design is forced by write rate and radius lookup cost, not by data size."},
        {k:"query",label:"Baseline nearby query",
          code:"SELECT driver_id, lat, lng\nFROM driver_locations\nWHERE lat BETWEEN :min_lat AND :max_lat\n  AND lng BETWEEN :min_lng AND :max_lng\n  AND ts >= now() - interval '15 seconds';"},
      ],
      snap:{title:"Load &amp; capacity — Stage 0",cap:"Pier 48 is correct at pilot size, but query cost still tracks global fleet size rather than nearby density.",
        tables:[{name:"signals",cols:["signal","value","verdict"],rows:[
          {c:["Pilot datastore","PostgreSQL/PostGIS **1 primary + 2 replicas**","N+1 pilot"],hi:1,tag:"hosted"},
          {c:["Production current drivers","~1M × ~150 B ≈ **150 MB**","fits RAM"],hi:1},
          {c:["Peak nearby scans","80K/s × 1M rows = **80B row checks/s**","breaks"],tag:"risk"},
          {c:["Driver updates","~250K/s vs 5–10K durable writes/node","breaks"],tag:"risk"},
          {c:["Freshness target","TTL ~10–15 s","ok"]},
        ]}]}},
    {node:"index",stage:"Stage 1 · Geo index",title:"Full scans cannot meet p99 &rarr; add spatial buckets",
      ...
      details:[
        {k:"pain",label:"What breaks — a concrete case",text:"At **22:08**, Pier 48 riders are all in three downtown H3 cells. Without a spatial index, every query still starts from the world. With fixed coarse cells, the opposite failure appears: one cell may contain **100K drivers**, so a single lookup returns too many candidates and one shard pins at 100%. A rider standing one block from a boundary can also miss the closest car if the query checks only the origin cell."},
        {k:"fix",label:"The fix — walk the same case, H3 cell rings",text:"Add a **Geo index** mapping `cell &rarr; driverIds`. A Pier 48 query computes the rider's H3 cell plus neighbor rings covering **2 km**, unions a bounded candidate set, hydrates current positions, then runs exact haversine and caps the nearest **N**. Dense SF cells use finer H3 resolution, around **65 m**, while sparse regions stay coarse. The 80K/s concert surge now does cell reads, not global scans, and border riders include neighbors by construction."},
        {k:"host",label:"Load &amp; capacity — what runs it",text:"Use **Redis GEO / Redis sets for the derived H3 index: 9 primary shards + 9 AZ replicas** for hot regions. Query math: **80K queries/s × ~9 cell lookups = ~720K small lookups/s**; **720K ÷ 9 = 80K ops/s/primary**, under the ~100K Redis ceiling. Why **9 not 8**: 8 primaries run at ~90K before hot-cell skew and failover promotion; 9 leaves safer headroom. Footprint is tiny, **~50–100 MB**, so shard count is lookup throughput and hotspot isolation, not RAM."},
        {k:"gotcha",label:"Density is the real shard key",text:"Fixed land-area cells fail in Pier 48 or Times Square and waste space in rural areas. Use adaptive resolution or density-based sharding so hot metros split finer while sparse regions stay coarse."},
      ],
      snap:{title:"Load &amp; capacity — Stage 1",cap:"Queries now scale with cells touched, and the Redis index is sized by lookup parallelism rather than bytes.",
        tables:[{name:"signals",cols:["signal","before","after"],rows:[
          {c:["Candidate source","~1M-row scan","~9 H3 cell lookups"],hi:1,tag:"fixed"},
          {c:["Geo index store","none","Redis GEO **9 primaries + 9 replicas**"],hi:1},
          {c:["Per-primary lookup load","not applicable","720K ÷ 9 ≈ **80K/s**"],hi:1},
          {c:["Index footprint","none","~50–100 MB derived"]},
    ...
    {node:"location",stage:"Stage 2 · Location ingest",title:"Moving drivers swamp the store &rarr; split the write firehose",
      ...
      details:[
        {k:"pain",label:"What breaks — a concrete case",text:"At **22:12**, 12K Pier 48 drivers start circling and the citywide driver fleet keeps reporting every **~4 s**. That is **1M drivers ÷ 4 s ≈ 250K location updates/s**. If every GPS sample performs a durable row update and re-index, the current-position store and geo index both take 250K/s writes. A brief deploy slowdown queues samples for **30 s**; riders are matched to cars that already left the curb, which is worse than dropping old samples."},
        {k:"fix",label:"The fix — walk the same case, dedicated ingest + hot memory store",text:"Add **Location ingest**. Driver frames are routed by `driver_id`, batched for **50–100 ms**, collapsed to the newest sample per driver, and written to an in-memory current-position store with TTL **~15 s**. The index is touched only when the driver changes H3 cell; because only **~1 in 15** updates crosses a cell boundary, index writes fall from **250K/s** to **~17K/s**. If a batch is lost, the next sample arrives in ~4 s; if a backlog forms, old samples are shed."},
        {k:"host",label:"Load &amp; capacity — what runs it",text:"Use **Redis GEO current-position store: 3 write primaries + 3 AZ replicas**, geo-sharded by density. Write math: **250K/s ÷ 3 ≈ 83K writes/s/primary**, below the ~100K ceiling; replicas give failover without WAL/fsync. Add **Kafka location-history: 48 partitions, RF3** for the durable analytics/billing fork. Kafka partitions are for **parallel consumers and per-driver ordering by key**, not byte volume; frames are only ~100 B, so bandwidth is ~25 MB/s. Ingest fleet is **~8 stateless nodes**, sized on connections and queue lag."},
        {k:"key",label:"Ephemeral by design",text:"Current position is a lease, not a ledger. If an update is lost, the next report arrives in ~4 s; durability belongs to the async history/billing fork, not the match-hot store."},
      ],
      snap:{title:"Load &amp; capacity — Stage 2",cap:"Writes stop competing with queries, and index updates become proportional to actual cell movement.",
        tables:[{name:"signals",cols:["signal","before","after"],rows:[
          {c:["Current-position store","shared durable writes","Redis GEO **3 primaries + 3 replicas**"],hi:1,tag:"hosted"},
          {c:["Per-primary writes","250K/s on one path","250K ÷ 3 ≈ **83K/s**"],hi:1},
          {c:["Index writes","~250K/s naive","~17K/s cell moves"],hi:1},
          {c:["Durable fork","none/on hot path","Kafka **48 partitions · RF3**"]},
    ...
    {node:"notify",stage:"Stage 3 · Dispatch notify",title:"Nearby list is not assignment &rarr; add dispatch and push",
      ...
      details:[
        {k:"pain",label:"What breaks — a concrete case",text:"At **22:20**, Pier 48 demand peaks at **50K ride requests/s**. If match simply broadcasts each request to the nearest 12 drivers, the system creates **600K offer messages/s** and a thundering accept race. A match pod crash loses thousands of in-flight offers, and two drivers tapping accept within **100 ms** can both believe they own the same rider. Location can be approximate, but assignment cannot."},
        {k:"fix",label:"The fix — walk the same case, dispatch and atomic claim",text:"Add **Dispatch / notify**. For each Pier 48 ride, dispatch offers sequentially or in tiny **2–3 driver** batches, stores short-TTL offer state externally, wakes phones through APNs/FCM when the socket is asleep, and finalizes the trip with a linearizable compare-and-set: `assignedDriver == null &rarr; driverId`. A pod crash delays offers until timeout or resume; duplicate accepts collapse to one winner."},
        {k:"host",label:"Load &amp; capacity — what runs it",text:"Run **~12 dispatch nodes** regionally: **50K requests/s ÷ ~5K/node ≈ 10**, plus N+1 and AZ spread. Offer state lives in **Redis 3 primaries + 3 replicas**; **150K live offers × ~1 KB ≈ 150 MB**, so memory is trivial and ops drive sizing. The authoritative ride claim uses a **DynamoDB regional table with 64 partitions** or equivalent linearizable KV: **50K conditional writes/s ÷ ~1K WCU/partition ≈ 50 partitions**, choose 64 for skew and failover. Partition by `ride_id`; dispatch shard by region so Pier 48 does not load every city."},
      ...
      snap:{title:"Load &amp; capacity — Stage 3 (full design)",cap:"The full Pier 48 design separates approximate proximity from exact assignment.",
        tables:[{name:"signals",cols:["concern","mechanism","result"],rows:[
          {c:["Dispatch fleet","~12 regional nodes · 3 AZs","N+1 at 50K/s"],hi:1},
          {c:["Offer state","Redis **3 primaries + 3 replicas** · ~150 MB live","N+1 TTL store"],hi:1,tag:"hosted"},
          {c:["Ride claim store","DynamoDB **64 partitions** · conditional update","one winner"],hi:1,tag:"fixed"},
          {c:["Why 64 partitions","50K/s ÷ 1K ≈ 50, plus skew headroom","ok"]},
