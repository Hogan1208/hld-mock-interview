...
window.DATA['video'] = {
  ...
  deepFlows:[
    {id:"upload-transcode-e2e",name:"Upload → ready",summary:"**POST /uploads** issues scoped part URLs → client uploads chunks **directly to object storage** → complete records <code>videos.status='uploaded'</code> → transcode fans one master into **6-8 renditions** → deterministic segment keys + metadata flip make the title playable.",steps:[
      {node:"client",title:"Client starts a resumable upload",narrate:"A creator's app does not stream a 50GB master through the application tier. It asks the upload control plane for a video id, a multipart session, and time-limited URLs for ~5-10MB chunks so a failure only retries missing parts.",details:[
      ...
      {node:"upload",title:"Upload service creates control-plane state",narrate:"The upload service authenticates, allocates a short <code>video_id</code>, opens the multipart session, and persists the resumable state. Any pod can later resume because the parts map is in shared metadata, not process memory.",details:[
      ...
      {node:"storage",title:"Client PUTs chunks to object storage",narrate:"The client uploads each part independently to the object store. Storage validates the checksum and returns an ETag; on a 90% failure of a 50GB file, the client asks for received parts and resends only the missing ~500 chunks.",details:[
      ...
      {node:"storage",title:"Workers write deterministic segments",narrate:"Each worker writes immutable HLS/DASH segments and a per-rendition manifest to deterministic keys. At-least-once queue delivery is safe because a duplicate worker overwrites the same output key with the same bytes or no-ops after seeing it exists.",details:[
    ...
    {id:"playback-e2e",name:"Playback path",summary:"Viewer hits play → metadata point-read by <code>video_id</code> builds the manifest → CDN edge serves immutable segments with **95-99% hit ratio** → cache misses coalesce through origin shield → ABR player chooses each segment and view counts update asynchronously.",steps:[
      {node:"client",title:"Viewer requests a video",narrate:"A viewer presses play on <code>v_9kQ2aZ</code>. The first request asks for structured metadata and a playback manifest, not for the video bytes themselves.",details:[
      ...
      {node:"meta",title:"Metadata DB point-reads by video_id",narrate:"The API routes the lookup to the local-region metadata partition for <code>video_id</code>. This is the playback-hot query: status plus the rendition map that says which manifest objects exist.",details:[
      ...
      {node:"cdn",title:"Return CDN manifest URLs",narrate:"The service builds or selects a device-specific HLS/DASH master manifest that points to CDN URLs, not storage-internal URLs. Device negotiation trims the ladder so an old phone never sees a 4K HEVC rung it cannot decode.",details:[
      ...
      {node:"player",title:"Player chooses the first rung",narrate:"The adaptive player fetches the manifest and starts conservatively: usually 1-2 small low/mid-rung segments to hit the startup SLO, then ramps based on buffer and measured throughput.",details:[
      ...
      {node:"cdn",title:"Segment cache hit or origin fill",narrate:"Segments are immutable cacheable objects. A normal hot segment is a local edge hit; a cold segment misses once, is fetched through shield/origin, then serves every other viewer in that PoP from cache.",details:[
      ...
      {node:"storage",title:"Origin storage serves misses durably",narrate:"On a miss, CDN pulls the segment from object storage. Object storage is not the source of metadata truth, but it is the source of bytes; it stores masters and renditions under deterministic keys and rebuilds redundancy after node loss.",details:[
      ...
      {node:"player",title:"ABR fetch loop continues",narrate:"Playback is now just repeated idempotent GETs. If throughput drops or the CDN 5xxs, the player retries the same segment on another CDN/edge and downshifts before its ~20-30s buffer drains.",details:[
      ...
      {node:"meta",title:"Count the view off the hot path",narrate:"The view event is not a synchronous <code>UPDATE view_counts SET count=count+1</code>. It is batched/aggregated asynchronously so a viral title at tens of thousands of views/s does not melt one metadata partition.",details:[
