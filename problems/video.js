...
var scaling={id:"scaling",name:"From raw file delivery to adaptive streaming",kind:"scale",
  ...
  steps:[
    {node:"storage",stage:"Stage 0 · Baseline",title:"Upload raw bytes, serve the original through CDN",
      ...
      details:[
        {k:"pain",label:"What breaks — a concrete case",text:"At **20:00**, creator Maya premieres **MoonDash Live Finale**, a 50 GB 4K master, and **2M fans** hit play in the first minute. The baseline serves the original file through CDN. Viewers on phones and weak hotel Wi-Fi are forced to pull the same **~16 Mbps** source; a 5 Mbps connection buffers after the first seconds, then retries the same oversized byte ranges. The CDN caches bytes, but the artifact is wrong: no small startup object, no lower bitrate, no device codec choice. A single viral title turns into millions of spinners even though storage itself is healthy."},
        {k:"fix",label:"The fix — walk the same case, durable raw delivery",text:"Keep the first invariant: upload bytes go **direct to object storage**, not through the app fleet. Maya's 50 GB master is multipart-uploaded in **~5000 parts** of 5–10 MB; if her connection dies at 90%, only the missing ~500 parts retry. The CDN can cache the raw object for MVP playback, and the upload service only handles control calls. The same premiere still buffers for many viewers, which is why Stage 1 changes the artifact, but Stage 0 makes the master durable and keeps ~300 Gbps sustained ingest off application servers."},
        {k:"host",label:"Load &amp; capacity — what runs it",text:"**S3-class object storage** stores masters with **3-AZ durability**; hot masters use 3 replicas, cold masters can use erasure coding later. Ingest math: **500 hours/min × ~10 Mbps ≈ 300 Gbps sustained**, peaking near **1 Tbps**, all direct-to-storage through pre-signed URLs. Upload control is only **~50 initiates/s + completes**, so **3 stateless upload pods per AZ** are for N+1, not bandwidth. CDN sits in front with origin shield; if edge hit is 95%, a 100 Gbps local premiere sends only **~5 Gbps** to origin."},
        {k:"wire",label:"Baseline objects",
          code:"POST /uploads -> upload_id\nPUT s3://masters/v_moondash_finale/source.mov\nGET https://cdn.example/masters/v_moondash_finale/source.mov"},
      ],
      snap:{title:"Load &amp; capacity — Stage 0",cap:"MoonDash is durable and cacheable, but one raw file cannot fit every device and network.",
        tables:[{name:"signals",cols:["signal","value","verdict"],rows:[
          {c:["Object store","S3-class · **3-AZ replicated hot masters**","N+1 durable"],hi:1,tag:"hosted"},
          {c:["Raw ingest","~300 Gbps sustained · ~1 Tbps peak","bypasses app tier"],hi:1},
          {c:["Upload control plane","~50 initiates/s + completes · 3 pods/AZ","N+1 not bandwidth"]},
          {c:["Playback artifact","one 16 Mbps original file","fragile"],tag:"risk"},
          {c:["CDN role","cache static file + origin shield","ok for MVP"]},
        ]}]}},
    {node:"transcode",stage:"Stage 1 · Transcode pipeline",title:"One huge file buffers &rarr; add renditions and segments",
      ...
      details:[
        {k:"pain",label:"What breaks — a concrete case",text:"At **20:03**, MoonDash viewers on commuter trains keep requesting the 4K source, then abandoning at **8–12 seconds** when the buffer drains. Smart TVs want HEVC, older browsers need H264, and mobile wants 480p or 720p. A worker crash during a naive whole-file encode of the **2-hour** master loses hours of work. With **500 hours/min** arriving platform-wide, inline processing would require users to wait behind CPU, and one bad deploy can leave new uploads unwatchable for hours."},
        {k:"fix",label:"The fix — walk the same case, async ladder generation",text:"Add **Transcoding**. MoonDash is split into **60 GOP-aligned 2-minute chunks** and **8 renditions**, yielding **480 independent jobs**. The first 240p/480p rungs publish quickly so viewers can start; 1080p and 4K fill in later. A failed spot worker retries only one chunk and overwrites the same deterministic segment key. The train viewer now pulls 480p segments at ~1.2 Mbps instead of the 16 Mbps master, and seeking jumps to the next small HLS/DASH segment."},
        {k:"host",label:"Load &amp; capacity — what runs it",text:"Use **Kafka transcode-jobs: 96 partitions, RF3**, plus an elastic worker pool. Partitions are for **ordering and consumer parallelism, not bytes**: job messages are tiny, but 96 partitions let many consumer groups drain titles concurrently while preserving retry visibility. Fleet math: **500 hours/min = 30,000 video-min/min**; at **~10 core-min per video-minute**, steady state is **~300K cores**, with peaks near **1M**. Object storage writes the ladder back under deterministic keys; hot renditions stay 3-replicated, cold renditions can be erasure-coded or regenerated from the master."},
      ...
      snap:{title:"Load &amp; capacity — Stage 1",cap:"Processing moves out of the upload request and becomes partitioned queue work over deterministic object keys.",
        tables:[{name:"signals",cols:["signal","before","after"],rows:[
          {c:["Transcode queue","none","Kafka **96 partitions · RF3**"],hi:1,tag:"hosted"},
          {c:["MoonDash fan-out","one 2h file","60 chunks × 8 rungs = **480 jobs**"],hi:1},
          {c:["Encode fleet","inline impossible","~300K cores steady · ~1M peak"],hi:1},
          {c:["Retry scope","whole file","one ~2-min chunk"]},
          {c:["Storage output","raw master only","6–8-rung segment ladder"],tag:"fixed"},
        ]}]}},
    {node:"meta",stage:"Stage 2 · Metadata service",title:"Bytes alone are not a product &rarr; add catalog metadata",
      ...
      details:[
        {k:"pain",label:"What breaks — a concrete case",text:"At **20:07**, MoonDash has 240p and 720p segments in object storage, but the player has no authoritative **ready** state or rendition map. Some edges infer readiness by listing object prefixes, which is slow and eventually inconsistent; others see the 4K folder first and build a manifest the mobile app cannot decode. Meanwhile every view tries to update one `view_counts` row for `v_moondash_finale`; **30K increments/s** serialize on a hot partition. Bytes exist, but product state is incoherent."},
        {k:"fix",label:"The fix — walk the same case, video_id metadata",text:"Add **Metadata DB** for upload sessions, `videos.status`, and the rendition map. When MoonDash's first playable rung lands, transcode writes rows keyed by `video_id` and flips status to **ready**; the player does one point read and receives only valid manifest URLs. View events go to Kafka and sharded counters instead of the hot video row. The same premiere now has a single source of truth for what can be played, while display counts lag by seconds without blocking playback."},
        {k:"host",label:"Load &amp; capacity — what runs it",text:"Use **Cassandra / ScyllaDB 12 nodes total, RF3 across regions** for core metadata. Size math: **8B rows × ~2 KB ≈ 16 TB**; across 12 nodes that is **~1.3 TB/node**, easy. Read math: **58K playback reads/s avg**, a few hundred K/s peak; at **~20K ops/s/node**, 3–4 nodes per serving region handle average, RF3 gives local quorum and failover. New video writes are gentle: **~4M/day ≈ 50/s**. View counts use **Kafka + 16 salted counter shards per hot video**, so MoonDash's 30K/s becomes ~1.9K/s per counter shard."},
        {k:"query",label:"Rendition lookup",
          code:"SELECT resolution, bitrate, codec, segment_manifest_url\nFROM renditions\nWHERE video_id = 'v_moondash_finale';"},
      ],
      snap:{title:"Load &amp; capacity — Stage 2",cap:"The database stores knowledge about MoonDash's bytes; the bytes stay in object storage and view counts stay off the hot row.",
        tables:[{name:"signals",cols:["signal","value","verdict"],rows:[
          {c:["Metadata DB","Cassandra/Scylla **12 nodes · RF3**","N+1 per region"],hi:1,tag:"hosted"},
          {c:["Rows per node","16 TB ÷ 12 ≈ **1.3 TB/node**","fits"],hi:1},
          {c:["Playback reads","58K/s avg · few hundred K/s peak","point-read + cache"]},
          {c:["Video writes","~4M/day = ~50/s","easy"]},
          {c:["Hot view counts","30K/s MoonDash ÷ 16 salts ≈ 1.9K/s/shard","safe"],hi:1},
        ]}]}},
    {node:"player",stage:"Stage 3 · Adaptive player",title:"Static files need client decisions &rarr; add ABR player",
      ...
      details:[
        {k:"pain",label:"What breaks — a concrete case",text:"At **20:15**, MoonDash is the top stream globally. One CDN PoP in Mumbai starts returning 5xx for every 1080p segment. Without an adaptive player, the server keeps handing out the same URLs and **500K viewers** stare at a spinner even though other CDNs and lower rungs are healthy. Server-side session steering would require tracking millions of playback sessions during a platform peak of **100–150M concurrent streams**, exactly where the delivery tier must remain stateless."},
        {k:"fix",label:"The fix — walk the same case, ABR and client telemetry",text:"Add the **Adaptive player**. Each MoonDash client starts with **1–2 low-rung segments**, builds a **20–30 s buffer**, and chooses the next segment from measured throughput, device codec support, and CDN errors. Mumbai viewers retry the same immutable segment on another CDN at the next boundary, downshift to 480p to refill, then ramp back up. The server stays a static file host; quality and failover are local decisions reported through QoE beacons."},
        {k:"host",label:"Load &amp; capacity — what runs it",text:"Delivery is **multi-CDN edge cache: hundreds of PoPs + origin shield + S3-class multi-region origins**. Peak math: **100–150M concurrent streams × ~5 Mbps ≈ ~500 Tbps**; at **99% edge hit**, origin sees ~**5 Tbps**, while 95% would be ~25 Tbps. That is why segments are immutable and cacheable for long TTLs. Metadata remains Cassandra **12 nodes RF3** for manifest point reads; playback session state is **0 server rows**, so scaling is CDN capacity, not database sessions."},
      ...
      snap:{title:"Load &amp; capacity — Stage 3 (full design)",cap:"The complete MoonDash path is stateless cache hits plus local client decisions, with origin protected by edge hit ratio.",
        tables:[{name:"signals",cols:["signal","value","verdict"],rows:[
          {c:["CDN tier","multi-CDN · hundreds of PoPs + origin shield","N+1 path"],hi:1,tag:"hosted"},
          {c:["Peak egress","~500 Tbps","served from edge"],hi:1},
          {c:["Origin load","500 Tbps × 1% miss ≈ **5 Tbps**","protected"],hi:1},
          {c:["Metadata DB","Cassandra **12 nodes · RF3**","manifest point-read"]},
          {c:["Server session rows","0","ABR state lives on client"],hi:1},
        ]}]}},
  ]};
