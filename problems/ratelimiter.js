...
var scaling={id:"scaling",name:"From one Redis counter to fleet-scale",kind:"scale",
  ...
  steps:[
    {node:"store",stage:"Stage 0 · Baseline",title:"One central Redis counter — exact fixed-window limiting",
      ...
      details:[
        {k:"pain",label:"What breaks — a concrete case",text:"At **00:00**, FlashCart starts its sneaker drop using API key `k_free_flashcart` on a free rule of **1000 req/min**. The first version is a single fixed-window Redis counter, so it is correct while traffic is calm: ~**1M requests/s** platform-wide across ~50 gateways, one `INCR` each. At **00:01**, partner bots retry aggressively but the counter still denies request **1001** for that key. The failure is not correctness yet; it is operational shape: every request, including obvious over-limit retries, still pays a network hop to Redis. If Redis latency moves from **0.8 ms to 4 ms**, the gateway decision p99 is already brushing the **5 ms** budget before the backend sees any work."},
        {k:"fix",label:"The fix — walk the same case, exact counter first",text:"Keep the honest baseline: all 50 gateways call a stateless limiter, and the limiter routes `k_free_flashcart` to exactly one Redis hash slot. Redis `INCR` returns the post-increment value, so request **1000** is allowed and **1001** is denied with no read-then-write race. FlashCart can retry 20K/s, but the answer is still globally exact. This stage buys the invariant; later stages make the same drop smoother and cheaper."},
        {k:"host",label:"Load &amp; capacity — what runs it",text:"**Redis Cluster: 12 primary shards + 12 replicas** across 3 AZs, hash-slotted by API key. Throughput math: **1M ops/s ÷ 12 ≈ 83K ops/s/primary**, below the ~100K simple-atomic ceiling. Why **12 not 10**: 10 primaries would run at ~100K each at peak with no CPU headroom for Lua, network jitter, or a hot-key burst; 12 leaves ~17% headroom. Each primary has one AZ replica for failover; memory is tiny: **10M counters × ~100 B ≈ 1 GB live**, or ~2 GB with two windows, so throughput, not RAM, sets the shard count."},
        {k:"query",label:"Baseline decision",
          code:"key = 'rl:api-key:{k_free_flashcart}:1718000460'\ncount = INCR key\nif count == 1: EXPIRE key 120\nallowed = count <= 1000\nreturn allowed, 1000 - count"},
      ],
      snap:{title:"Load &amp; capacity — Stage 0",cap:"FlashCart is globally exact on a single Redis cluster, but every decision still pays the central hop.",
        tables:[{name:"signals",cols:["signal","value","verdict"],rows:[
          {c:["Counter store","Redis Cluster · **12 primaries + 12 replicas** · 3 AZs","N+1 failover"],hi:1,tag:"hosted"},
          {c:["Per-primary load","1M ÷ 12 ≈ **83K ops/s** vs ~100K ceiling","headroom"],hi:1},
          {c:["Why 12 not 10","10 primaries ≈ 100K each at peak","too tight"]},
          {c:["Memory","10M counters × ~100 B ≈ **1 GB** live, ~2 GB with two windows","not limiting"]},
          {c:["Decision p99","inside **5 ms** while Redis RTT is healthy","ok"]},
        ]}]}},
    {node:"algo",stage:"Stage 1 · Smooth algorithm",title:"Fixed windows leak 2× bursts &rarr; add the algorithm engine",
      ...
      details:[
        {k:"pain",label:"What breaks — a concrete case",text:"FlashCart learns the boundary trick. At **12:00:59.200**, its mobile clients send **1000** checkout calls and drain the free bucket. At **12:01:00.050**, the fixed window resets and another **1000** calls pass. The limiter can claim both minutes are legal, but the checkout backend receives **2000 requests in under 1 second** from a customer promised **1000/min**. When 200 similar keys synchronize on the minute, a protected service sized for smooth traffic sees a reset-boundary herd and p99 jumps from **80 ms to seconds**."},
        {k:"fix",label:"The fix — walk the same case, smooth limiting",text:"Add the **Algorithm engine** and make FlashCart's rule a token bucket or sliding-window-counter. The exact same **1000 at 12:00:59 + 1000 at 12:01:00** attempt is blended against the trailing window, so only the configured burst capacity passes and the rest get **429 + Retry-After**. The math still executes as one Redis Lua script: refill, check, decrement, and TTL update are atomic on the owning shard, so smoothing does not reintroduce a race."},
        {k:"host",label:"Load &amp; capacity — what runs it",text:"The engine is stateless CPU inside the limiter; durable state remains **Redis Cluster 12 primaries + 12 replicas**. Token bucket state is **2 numbers/key** (`tokens`, `last_refill_ts`), roughly **10M × 32 B ≈ 320 MB** plus overhead; sliding-window-counter is similarly **2 counters/key**. Avoid sliding-window-log: **10M keys × 1000 timestamps ≈ 10B entries**, hundreds of GB before allocator overhead. Lua reduces per-shard headroom, so the Stage-0 choice of 12 primaries matters: **1M ÷ 12 ≈ 83K Lua ops/s**, still below a conservative ~90K/simple-script ceiling."},
        {k:"gotcha",label:"Avoid the exact log trap",text:"Sliding-window-log is exact, but it stores one timestamp per request. FlashCart's free rule is about load protection, not billing to the request, so O(1) counters are the correct trade: bounded burst error, stable memory."},
      ],
      snap:{title:"Load &amp; capacity — Stage 1",cap:"The same Redis cluster now stores O(1) smooth-bucket state instead of fixed buckets or per-request logs.",
        tables:[{name:"signals",cols:["signal","before","after"],rows:[
          {c:["Boundary burst","up to **2×** at minute reset","token bucket / sliding counter"],hi:1,tag:"fixed"},
          {c:["Counter store","Redis **12 primaries + 12 replicas**","same N+1 cluster"],hi:1},
          {c:["Per-key state","1 fixed counter","2 numbers/key · O(1)"],hi:1},
    ...
    {node:"config",stage:"Stage 2 · Runtime rules",title:"One hard-coded limit cannot serve tiers &rarr; add config",
      ...
      details:[
        {k:"pain",label:"What breaks — a concrete case",text:"At **12:08**, FlashCart pays for an enterprise rescue because the sneaker queue is legitimate. Support promises to raise `k_free_flashcart` from **1000/min** to **5M/min** immediately. In the hard-coded design, 80 gateway and limiter nodes keep the old value until a deploy rolls through. For **12 minutes**, half the fleet denies FlashCart at 1000/min while the other half, already redeployed, allows more; the customer sees random 429s and retries harder. A later fat-finger of `0/min` would propagate only by another deploy, making rollback just as slow."},
        {k:"fix",label:"The fix — walk the same case, versioned config",text:"Add **Rules / config** as a versioned control plane. The FlashCart override is written as version **v1842**, canaried to 5% of nodes, then pushed to all subscribers in **~1–2 s**. Each node hot-swaps the immutable ruleset, reports `enforcing=v1842`, and boots from last-known-good if config is unreachable. The same emergency raise now converges while the drop is still live, and rollback is publishing v1841, not redeploying binaries."},
        {k:"host",label:"Load &amp; capacity — what runs it",text:"Use **etcd: 3 voting nodes across 3 AZs**, plus git/review as the human audit trail. The ruleset is only **~1 MB**: tiers plus **~10K overrides × ~100 B**. Watch load is tiny: **~80 subscribers × 1 MB** on a change is ~80 MB of fan-out, rare compared with 1M request/s on the hot path. Why **3 not 2**: etcd needs quorum; with 3 voters, one AZ can fail and **2/3** still elect a leader. Nodes cache rules in memory, so per-request lookup is a microsecond hash lookup, not an etcd read."},
      ...
      snap:{title:"Load &amp; capacity — Stage 2",cap:"Config is not sized by request QPS; it is sized by safe, observable propagation to the nodes enforcing FlashCart.",
        tables:[{name:"signals",cols:["signal","value","verdict"],rows:[
          {c:["Config store","etcd **3 voters / 3 AZs**","quorum N+1"],hi:1,tag:"hosted"},
          {c:["Rule-set size","~1 MB + ~10K overrides","cache everywhere"],hi:1},
          {c:["Subscribers","~80 gateway + limiter nodes","push + poll backstop"]},
          {c:["Propagation","~1–2 s fleet-wide","fast enough"],hi:1,tag:"fixed"},
          {c:["Hot path","in-memory rule lookup","no config read/request"]},
        ]}]}},
    {node:"sync",stage:"Stage 3 · Local budgets",title:"Redis round-trips cap latency &rarr; add cluster sync",
      ...
      details:[
        {k:"pain",label:"What breaks — a concrete case",text:"At **12:15**, FlashCart's enterprise key is legitimately doing **400K requests/s** during checkout polling. Strict-central counting routes every request for that key to one Redis owner shard. Even though the whole cluster has 12 primaries, that single key pins one primary near **400K ops/s**, roughly **4×** its comfortable ceiling, while other shards are idle. Limiter instances wait on Redis, decision p99 crosses **8 ms**, gateways pile up, and healthy customers sharing that shard see collateral 429s/timeouts."},
        {k:"fix",label:"The fix — walk the same case, leased local budgets",text:"Add **Cluster sync** for high-volume tiers only. FlashCart's **5M/min** allowance is leased as short-lived token slices to the ~50 limiter nodes proportional to recent traffic. Hot nodes spend local memory tokens in microseconds and report deltas every **~500 ms**; the aggregator refreshes budgets and folds totals into Redis. If sync partitions, leases expire after ~2 s and nodes fall back to strict-central or a conservative floor. FlashCart's 400K/s is now spread across nodes instead of one Redis shard, while free-tier `1000/min` keys remain exact."},
        {k:"host",label:"Load &amp; capacity — what runs it",text:"Run **1 active sync aggregator + 1 hot standby**, backed by the authoritative **Redis 12+12** counter store. Report math: **50 limiter nodes × 2 reports/s = ~100 reports/s**, each batched across active high-volume keys, trivial for one process. Budget state worst case is **10M active keys × ~50 B ≈ 500 MB**, but the real tracked set is much smaller because only high-volume keys lease locally. Limiter fleet math improves from **~25K strict decisions/s/instance** to **~200K local decisions/s/instance**; 1M/s needs ~5–8 hot-path instances plus redundancy, while the existing ~40–50 nodes provide AZ headroom."},
      ...
      snap:{title:"Load &amp; capacity — Stage 3 (full design)",cap:"The final design chooses strict Redis atomics or local leased budgets per tier instead of pretending one accuracy mode fits every FlashCart-shaped spike.",
        tables:[{name:"signals",cols:["concern","strict-central","local + sync"],rows:[
          {c:["Counter store","Redis **12 primaries + 12 replicas**","same authority"],hi:1},
          {c:["Hot-key ceiling","one key can pin one shard","budget spread across ~50 nodes"],hi:1,tag:"fixed"},
          {c:["Per-instance throughput","~25K/s","~200K/s"],hi:1},
          {c:["Sync service","none","1 active + 1 standby · ~100 reports/s"],tag:"N+1"},
          {c:["Correctness","exact","bounded overshoot by lease"],hi:1},
        ]}]}},
  ]};
