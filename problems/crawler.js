...
window.DATA['crawler'] = {
  ...
  deepFlows:[
    {id:"crawl-loop-e2e",name:"Crawl loop",summary:"**frontier_queue** leases the next host-eligible URL → DNS cache hit/miss → **host_politeness** gates robots + crawl-delay → async fetch → raw HTML reaches the parser → object storage + **pages** metadata commit.",steps:[
      {node:"frontier",title:"Lease the next polite URL",narrate:"The frontier is not FIFO: it picks a high-priority URL whose host is eligible to crawl now. Because each host maps to exactly one shard with <code>worker = hash(host) % 200</code>, the host's queue, in-flight lease, and rate state are local.",details:[
      ...
      {node:"dns",title:"Resolve the host",narrate:"Before opening a socket, the fetcher asks the resolver cache for the host's IP. At ~12K pages/s, resolving every page would be fatal; the crawler expects >99% hits because the same hosts are revisited constantly.",details:[
      ...
      {node:"politeness",title:"Gate on robots and crawl-delay",narrate:"The fetcher must prove the URL is allowed and the host's minimum interval has elapsed. If <code>robots.txt</code> is stale or unknown, the safe behavior is to defer or fetch robots first, not crawl blind.",details:[
      ...
      {node:"fetcher",title:"Fetch with bounded resources",narrate:"The fetcher uses async I/O, timeouts, redirect limits, and a ~10MB body cap. It sends a clear crawler User-Agent and backs off immediately on <code>429</code> or <code>Retry-After</code>.",details:[
      ...
      {node:"parser",title:"Parse raw HTML and extract signals",narrate:"The parser receives raw bytes, extracts visible text and outlinks, canonicalizes URLs, and computes content fingerprints. Static HTML parsing is the default path; JS rendering is reserved for domains that prove they need it.",details:[
      ...
      {node:"store",title:"Persist bytes and crawl metadata",narrate:"The raw compressed page is packed into WARC/object storage; the small lookup row lands in the <code>pages</code> metadata index. The object key is derived from the URL hash and crawl time so writes are idempotent and versions do not overwrite each other.",details:[
    ...
    {id:"discover-enqueue-e2e",name:"Discover links",summary:"Parser emits ~10 links/page → canonicalize → **Bloom + seen_urls** prevents URL re-crawl → trap rules cap infinite spaces → new URLs are inserted into **frontier_queue** on the host shard with priority and earliest polite time.",steps:[
      {node:"parser",title:"Canonicalize extracted outlinks",narrate:"A fetched page can produce dozens of spellings for the same target URL. Canonicalization happens before dedup so the frontier only ever sees one key for one page.",details:[
      ...
      {node:"dedup",title:"Probe the Bloom filter",narrate:"The hot URL-seen check is probabilistic and local to the host shard. With ~30B URLs, the Bloom filter is the difference between a ~37GB fleet-wide structure and a multi-TB exact set.",details:[
      ...
      {node:"frontier",title:"Apply trap and admission policy",narrate:"Dedup only catches repeats; spider traps create truly unique URLs. Before insert, the frontier enforces depth, per-domain budget, and pattern rules so one calendar/faceted site cannot fill the 3B-URL frontier alone.",details:[
      ...
      {node:"frontier",title:"Insert new URL on the host shard",narrate:"A genuinely new, admitted URL is inserted into <code>frontier_queue</code> with its canonical URL, host, priority, depth, and earliest fetch time. The scheduled time is at least the host's next polite slot.",details:[
    ...
    {id:"index-dedup-e2e",name:"Index dedup",summary:"Parsed content gets a **content_hash / SimHash** → exact and near-duplicate probes avoid re-indexing boilerplate clones → unique pages write WARC bytes plus the **pages** row, while duplicates point at the canonical copy.",steps:[
      {node:"parser",title:"Fingerprint normalized content",narrate:"After boilerplate stripping, the parser computes fingerprints over the extracted text: an exact checksum for byte-equivalent content and a SimHash for near-duplicates like print views, mirrors, and pages differing only by ads.",details:[
      ...
      {node:"dedup",title:"Probe exact content duplicates",narrate:"The metadata index is small enough for fast point lookups. A content hash hit means the bytes/content are already represented in the corpus, so the indexer can attach this URL as a duplicate instead of re-indexing another copy.",details:[
      ...
      {node:"dedup",title:"Probe near-duplicates with LSH buckets",narrate:"For SimHash, brute-forcing against billions of pages is impossible. Split the fingerprint into bands and only compare candidates sharing a band; it is approximate but catches the duplicate patterns that waste index space and reveal crawler traps.",details:[
      ...
      {node:"store",title:"Write the unique raw page",narrate:"If the page is unique enough, store the compressed raw HTML in WARC/object storage. Packing many pages per object avoids the HDFS-style small-file metadata wall for 30B pages.",details:[
      ...
      {node:"store",title:"Upsert the pages index row",narrate:"The search-index pipeline and future recrawls read the <code>pages</code> metadata row by <code>url_hash</code>. It records the latest <code>content_hash</code>, status, object key, and fetch timestamp using the exact schema fields.",details:[
      ...
      {node:"store",title:"Hand off to indexing without blocking crawl",narrate:"The crawler should not stall if the downstream indexer is slow. Store commit is the durable boundary; indexing can consume pages or WARC batches asynchronously and replay from the object store if its extraction logic changes.",details:[
