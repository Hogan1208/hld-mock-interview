...
window.DATA['filesync'] = {
  ...
  deepFlows:[
    {id:"upload-e2e",name:"Upload a change",summary:"Client detects a local edit → **content-defined chunks** it → asks which hashes are missing → uploads only new chunks keyed by **content hash** → commits <code>files.current_version</code> + <code>file_versions.chunk_hashes</code> atomically → sends a best-effort poke.",steps:[
      {node:"client",title:"Sync agent sees plan.xlsx change",snap:{cap:"Local edit only; no server mutation yet. The authoritative row on <strong>metadata shard ns42</strong> still points at base version 6 while device 5502 prepares a candidate v7.",tables:[{name:"files (metadata shard ns42)",cols:["file_id","namespace_id","path","current_version","is_dir"],rows:[{c:["9001","42","/plan.xlsx","6","false"],hi:1,tag:"base"}]},{name:"device_cursors (metadata shard ns42)",cols:["device_id","namespace_id","last_synced_version"],rows:[{c:["5502","42","6"],hi:1,tag:"editing from"},{c:["5501","42","6"]}]}]},narrate:"The desktop agent receives a filesystem event for <code>/plan.xlsx</code>, debounces it with nearby edits, and compares the local index against the last server version it applied. The server remains authoritative; the client is only preparing a candidate delta.",details:[
      ...
      {node:"chunk",title:"Chunk and hash locally",snap:{cap:"The client has produced an ordered content-addressed manifest for candidate v7. This step is local compute only; the DB still shows v6 and no chunk refs have changed.",tables:[{name:"file_versions (metadata shard ns42)",cols:["file_id","version","chunk_hashes","size","mtime","device_id"],rows:[{c:["9001","6","[c1a2, c9f3, cb44]","1050112","2026-07-22 09:30:00","5502"],hi:1,tag:"base read"},{c:["9001","7","[c1a2, c9f3, ce77]","1054208","2026-07-25 07:40:11","5502"],tag:"candidate"}]},{name:"chunks (dedup index)",cols:["chunk_hash","storage_url","ref_count","size"],rows:[{c:["c1a2","s3://blk/c1/c1a2","2","524288"]},{c:["c9f3","s3://blk/c9/c9f3","2","524288"]},{c:["ce77","<em>not present yet</em>","0","525824"],hi:1,tag:"new hash"}]}]},narrate:"The client runs content-defined chunking with a ~4MB target so an insertion changes only the chunk around the edit, not every offset after it. It computes strong hashes and orders them exactly as the file should be reassembled.",details:[
      ...
      {node:"gw",title:"Ask which chunks are missing",snap:{cap:"Read-only hash existence check on the dedup map: <code>c1a2</code> and <code>c9f3</code> are reusable, <code>ce77</code> is missing so the API returns one scoped upload URL. No metadata mutation yet.",tables:[{name:"chunks (dedup index)",cols:["chunk_hash","storage_url","ref_count","size"],rows:[{c:["c1a2","s3://blk/c1/c1a2","2","524288"],hi:1,tag:"present"},{c:["c9f3","s3://blk/c9/c9f3","2","524288"],hi:1,tag:"present"},{c:["ce77","<em>absent &rarr; mint PUT URL</em>","0","525824"],hi:1,tag:"missing"}]},{name:"files (metadata shard ns42)",cols:["file_id","namespace_id","path","current_version","is_dir"],rows:[{c:["9001","42","/plan.xlsx","6","false"],tag:"unchanged"}]}]},narrate:"The API + sync service authenticates the device and forwards a cheap hash-existence check. It returns upload URLs only for chunk hashes absent from the <code>chunks</code> table; already-known hashes are just referenced in the eventual version commit.",details:[
      ...
      {node:"block",title:"Upload only new chunks",snap:{cap:"Only missing chunk <code>ce77</code> is now durable in block storage under its content hash. It is still unreferenced by metadata until the version commit succeeds.",tables:[{name:"chunks (dedup index)",cols:["chunk_hash","storage_url","ref_count","size"],rows:[{c:["c1a2","s3://blk/c1/c1a2","2","524288"]},{c:["c9f3","s3://blk/c9/c9f3","2","524288"]},{c:["ce77","s3://blk/ce/ce77","0","525824"],hi:1,tag:"uploaded orphan"}]},{name:"file_versions (metadata shard ns42)",cols:["file_id","version","chunk_hashes","size","mtime","device_id"],rows:[{c:["9001","7","[c1a2, c9f3, ce77]","1054208","2026-07-25 07:40:11","5502"],tag:"not committed"}]}]},narrate:"The client uploads missing chunk <code>ce77</code> directly to block storage; bytes never traverse the metadata fleet. The object key is derived from the content hash, so retries are idempotent and duplicate uploads converge on the same immutable object.",details:[
      ...
      {node:"meta",title:"Commit the new file version",snap:{cap:"Control-plane commit request reaches the namespace shard. The proposed CAS is <code>base_version 6 &rarr; new_version 7</code>; it has not mutated rows until the DB transaction runs.",tables:[{name:"files (metadata shard ns42)",cols:["file_id","namespace_id","path","current_version","is_dir"],rows:[{c:["9001","42","/plan.xlsx","6","false"],hi:1,tag:"CAS expects 6"}]},{name:"file_versions (metadata shard ns42)",cols:["file_id","version","chunk_hashes","size","mtime","device_id"],rows:[{c:["9001","6","[c1a2, c9f3, cb44]","1050112","2026-07-22 09:30:00","5502"]},{c:["9001","7","[c1a2, c9f3, ce77]","1054208","2026-07-25 07:40:11","5502"],hi:1,tag:"pending insert"}]}]},narrate:"After all referenced chunks are durable, the client sends a small metadata commit. This is the strong-consistency boundary: advance the file's live pointer only if it still points at the base version the client edited.",details:[
      ...
      {node:"db",title:"CAS update files + insert file_versions",snap:{cap:"The namespace-shard transaction succeeds: compare-and-set changes <code>files.current_version 6&rarr;7</code>, inserts immutable v7 history, and increments refs for the chunks in the manifest.",tables:[{name:"files (metadata shard ns42)",cols:["file_id","namespace_id","path","current_version","is_dir"],rows:[{c:["9001","42","/plan.xlsx","7","false"],hi:1,tag:"CAS success"}]},{name:"file_versions (metadata shard ns42)",cols:["file_id","version","chunk_hashes","size","mtime","device_id"],rows:[{c:["9001","6","[c1a2, c9f3, cb44]","1050112","2026-07-22 09:30:00","5502"]},{c:["9001","7","[c1a2, c9f3, ce77]","1054208","2026-07-25 07:40:11","5502"],hi:1,tag:"inserted"}]},{name:"chunks (dedup index)",cols:["chunk_hash","storage_url","ref_count","size"],rows:[{c:["c1a2","s3://blk/c1/c1a2","3","524288"],hi:1,tag:"ref +1"},{c:["c9f3","s3://blk/c9/c9f3","3","524288"],hi:1,tag:"ref +1"},{c:["ce77","s3://blk/ce/ce77","1","525824"],hi:1,tag:"referenced"}]}]},narrate:"The metadata DB verifies every chunk hash exists, inserts the immutable <code>file_versions</code> row, and advances <code>files.current_version</code> from 6 to 7 with a compare-and-set. If another device already advanced the file, this update affects zero rows and becomes a conflict path.",details:[
      ...
      {node:"notif",title:"Poke subscribed devices",snap:{cap:"After durable metadata commit, subscribers are merely hinted. Device 5501 remains at cursor 6 until it pulls; the authoritative file row is already v7 on shard ns42.",tables:[{name:"files (metadata shard ns42)",cols:["file_id","namespace_id","path","current_version","is_dir"],rows:[{c:["9001","42","/plan.xlsx","7","false"],hi:1,tag:"durable"}]},{name:"device_cursors (metadata shard ns42)",cols:["device_id","namespace_id","last_synced_version"],rows:[{c:["5502","42","7"],tag:"writer caught up"},{c:["5501","42","6"],hi:1,tag:"stale"}]}]},narrate:"Once the metadata transaction commits, the system publishes a tiny hint to devices subscribed to namespace 42. The notification is intentionally not the source of truth; it only tells devices to pull from their cursor.",details:[
    ...
    {id:"download-e2e",name:"Sync down change",summary:"A device receives a **namespace advanced** poke → asks for metadata since <code>device_cursors.last_synced_version</code> → reads <code>files</code> + <code>file_versions.chunk_hashes</code> on the namespace shard → downloads missing content-hash chunks → atomically advances its device cursor.",steps:[
      {node:"notif",title:"Phone receives a stale hint",snap:{cap:"The poke is a best-effort hint only. Phone device 5501 is behind at cursor 6 while the namespace shard already has <code>/plan.xlsx</code> at v7; no state changes on receipt.",tables:[{name:"device_cursors (metadata shard ns42)",cols:["device_id","namespace_id","last_synced_version"],rows:[{c:["5501","42","6"],hi:1,tag:"behind"},{c:["5502","42","7"]}]},{name:"files (metadata shard ns42)",cols:["file_id","namespace_id","path","current_version","is_dir"],rows:[{c:["9001","42","/plan.xlsx","7","false"],hi:1,tag:"hint target"}]}]},narrate:"The phone is subscribed to namespace 42 and currently has <code>last_synced_version=6</code>. A poke says <code>/plan.xlsx</code> is now at version 7, so it schedules a pull; a duplicate poke would schedule the same idempotent pull again.",details:[
      ...
      {node:"client",title:"Client asks changes since its cursor",snap:{cap:"Read-only cursor presentation: device 5501 asks for all namespace-42 versions after 6. The cursor row is highlighted because it determines the delta window; it is not advanced yet.",tables:[{name:"device_cursors (metadata shard ns42)",cols:["device_id","namespace_id","last_synced_version"],rows:[{c:["5501","42","6"],hi:1,tag:"read cursor"}]},{name:"file_versions (metadata shard ns42)",cols:["file_id","version","chunk_hashes","size","mtime","device_id"],rows:[{c:["9001","7","[c1a2, c9f3, ce77]","1054208","2026-07-25 07:40:11","5502"],tag:"eligible delta"}]}]},narrate:"The phone presents its durable cursor for this namespace. If it was offline for minutes, the same request returns all missed versions in order; if the poke was duplicate, the response is empty.",details:[
      ...
      {node:"gw",title:"Route pull to the namespace shard",snap:{cap:"Routing by <code>namespace_id=42</code> sends the pull to the same shard that serialized the commit. The cached key <code>(42,6)</code> is immutable after v7 commits; this is still a read-only step.",tables:[{name:"namespaces (routing)",cols:["namespace_id","owner_id","type"],rows:[{c:["42","7","user"],hi:1,tag:"hash &rarr; shard ns42"}]},{name:"device_cursors (metadata shard ns42)",cols:["device_id","namespace_id","last_synced_version"],rows:[{c:["5501","42","6"],hi:1,tag:"since=6"}]}]},narrate:"The API + sync service authenticates the device and routes the read by <code>namespace_id=42</code>, the same key used by commits. This keeps changes-since a single-shard read and makes cache keys naturally <code>(namespace_id, cursor)</code>.",details:[
      ...
      {node:"meta",title:"Build the delta manifest",snap:{cap:"Read-only delta build: the service joins the live file pointer to version 7 and returns the ordered chunk list. The highlighted v7 row is what every device at cursor 6 will see.",tables:[{name:"files (metadata shard ns42)",cols:["file_id","namespace_id","path","current_version","is_dir"],rows:[{c:["9001","42","/plan.xlsx","7","false"],hi:1,tag:"changed file"}]},{name:"file_versions (metadata shard ns42)",cols:["file_id","version","chunk_hashes","size","mtime","device_id"],rows:[{c:["9001","7","[c1a2, c9f3, ce77]","1054208","2026-07-25 07:40:11","5502"],hi:1,tag:"manifest"}]}]},narrate:"The metadata service loads the changed file row and its new version manifest. In the simplified schema the file's live pointer is <code>files.current_version</code>, and the ordered content list is <code>file_versions.chunk_hashes</code>.",details:[
      ...
      {node:"db",title:"Return exact chunk locations",snap:{cap:"Read-only resolution of manifest hashes to block locations. Present chunks may already be cached locally; the response tells the phone exactly where each content hash lives.",tables:[{name:"chunks (dedup index)",cols:["chunk_hash","storage_url","ref_count","size"],rows:[{c:["c1a2","s3://blk/c1/c1a2","3","524288"],hi:1,tag:"location"},{c:["c9f3","s3://blk/c9/c9f3","3","524288"],hi:1,tag:"location"},{c:["ce77","s3://blk/ce/ce77","1","525824"],hi:1,tag:"location"}]},{name:"device_cursors (metadata shard ns42)",cols:["device_id","namespace_id","last_synced_version"],rows:[{c:["5501","42","6"],tag:"not advanced"}]}]},narrate:"The DB path resolves the ordered hashes to storage locations and sizes. The client can skip any chunk it already has locally by hash; identical chunks across files and versions need no download.",details:[
      ...
      {node:"block",title:"Download missing chunks by hash",snap:{cap:"The phone reuses chunks it already has and downloads only <code>ce77</code>. Server metadata does not mutate during the block read; hash verification protects assembly.",tables:[{name:"chunks (block read)",cols:["chunk_hash","storage_url","ref_count","size"],rows:[{c:["c1a2","s3://blk/c1/c1a2","3","524288"],tag:"local hit"},{c:["c9f3","s3://blk/c9/c9f3","3","524288"],tag:"local hit"},{c:["ce77","s3://blk/ce/ce77","1","525824"],hi:1,tag:"downloaded"}]},{name:"device_cursors (metadata shard ns42)",cols:["device_id","namespace_id","last_synced_version"],rows:[{c:["5501","42","6"],tag:"still 6"}]}]},narrate:"The phone compares the manifest to its local chunk cache. It already has <code>c1a2</code> and <code>c9f3</code>, so it fetches only <code>ce77</code> from block storage and verifies the bytes hash to that key.",details:[
      ...
      {node:"client",title:"Assemble file and advance cursor",snap:{cap:"After bytes are durable locally, the device cursor CAS advances <code>6&rarr;7</code>. If the client crashed before this row changed, it would safely repeat the same pull.",tables:[{name:"device_cursors (metadata shard ns42)",cols:["device_id","namespace_id","last_synced_version"],rows:[{c:["5501","42","7"],hi:1,tag:"cursor advanced"},{c:["5502","42","7"]}]},{name:"files (metadata shard ns42)",cols:["file_id","namespace_id","path","current_version","is_dir"],rows:[{c:["9001","42","/plan.xlsx","7","false"],tag:"applied"}]}]},narrate:"The client writes the new file to a temp local path, verifies the full manifest, atomically swaps it into <code>/plan.xlsx</code>, and only then advances its cursor. A crash before cursor update simply repeats the pull safely.",details:[
    ...
    {id:"conflict-e2e",name:"Offline conflict",summary:"Two devices edit from the same base → both upload chunks safely → the first CAS advances <code>files.current_version</code> → the second CAS fails → it creates a **conflicted copy** as a new file/version instead of last-write-wins data loss.",steps:[
      {node:"client",title:"Two devices edit from base v6",snap:{cap:"Both devices fork from the same durable base v6. The server has not accepted either edit; conflict detection will be based on base_version vs <code>files.current_version</code>, not timestamps.",tables:[{name:"files (metadata shard ns42)",cols:["file_id","namespace_id","path","current_version","is_dir"],rows:[{c:["9001","42","/plan.xlsx","6","false"],hi:1,tag:"shared base"}]},{name:"device_cursors (metadata shard ns42)",cols:["device_id","namespace_id","last_synced_version"],rows:[{c:["5502","42","6"],hi:1,tag:"laptop"},{c:["5501","42","6"],hi:1,tag:"desktop"}]}]},narrate:"Laptop <code>5502</code> and desktop <code>5501</code> both last synced <code>/plan.xlsx</code> at version 6, then go offline and make different edits. Timestamps are not trusted for conflict resolution because device clocks skew.",details:[
      ...
      {node:"block",title:"Both upload their new chunks",snap:{cap:"Both unique chunks become durable before either metadata decision. <code>ce77</code> and <code>cf88</code> are uploaded as unreferenced content-addressed blocks; old chunks remain shared by hash.",tables:[{name:"chunks (dedup index)",cols:["chunk_hash","storage_url","ref_count","size"],rows:[{c:["c1a2","s3://blk/c1/c1a2","2","524288"],tag:"reused"},{c:["c9f3","s3://blk/c9/c9f3","2","524288"],tag:"reused"},{c:["ce77","s3://blk/ce/ce77","0","525824"],hi:1,tag:"laptop uploaded"},{c:["cf88","s3://blk/cf/cf88","0","524912"],hi:1,tag:"desktop uploaded"}]},{name:"files (metadata shard ns42)",cols:["file_id","namespace_id","path","current_version","is_dir"],rows:[{c:["9001","42","/plan.xlsx","6","false"],tag:"no winner yet"}]}]},narrate:"Each device uploads only its unique changed chunk (<code>ce77</code> or <code>cf88</code>) through the same content-addressed block path. Duplicate old chunks are reused by hash, so conflict handling does not double-store the whole file.",details:[
      ...
      {node:"meta",title:"First commit wins the fast-forward",snap:{cap:"Laptop 5502 wins the serialization point: CAS sees current_version 6 and advances <code>9001</code> to v7, then inserts the v7 manifest that references <code>ce77</code>.",tables:[{name:"files (metadata shard ns42)",cols:["file_id","namespace_id","path","current_version","is_dir"],rows:[{c:["9001","42","/plan.xlsx","7","false"],hi:1,tag:"CAS 6&rarr;7"}]},{name:"file_versions (metadata shard ns42)",cols:["file_id","version","chunk_hashes","size","mtime","device_id"],rows:[{c:["9001","6","[c1a2, c9f3, cb44]","1050112","2026-07-22 09:30:00","5502"]},{c:["9001","7","[c1a2, c9f3, ce77]","1054208","2026-07-25 07:40:11","5502"],hi:1,tag:"winner"}]},{name:"chunks (dedup index)",cols:["chunk_hash","storage_url","ref_count","size"],rows:[{c:["ce77","s3://blk/ce/ce77","1","525824"],hi:1,tag:"ref +1"},{c:["cf88","s3://blk/cf/cf88","0","524912"],tag:"orphan for now"}]}]},narrate:"The laptop reconnects first. Its commit says base version 6, and the DB still has <code>files.current_version=6</code>, so the compare-and-set advances the file to version 7.",details:[
      ...
      {node:"db",title:"Second commit sees stale base",snap:{cap:"Desktop 5501 tries the same CAS from base 6, but the highlighted live row is already v7. The guarded update affects zero rows, so this is a stale-base failure, not an overwrite.",tables:[{name:"files (metadata shard ns42)",cols:["file_id","namespace_id","path","current_version","is_dir"],rows:[{c:["9001","42","/plan.xlsx","7","false"],hi:1,tag:"stale base 6 rejected"}]},{name:"file_versions (metadata shard ns42)",cols:["file_id","version","chunk_hashes","size","mtime","device_id"],rows:[{c:["9001","7","[c1a2, c9f3, ce77]","1054208","2026-07-25 07:40:11","5502"],hi:1,tag:"server winner"}]}]},narrate:"The desktop reconnects seconds later with base version 6, but the live row is already version 7. The guarded update affects zero rows, so the metadata service rejects it as a stale fast-forward rather than overwriting the laptop's version.",details:[
      ...
      {node:"meta",title:"Materialize a conflicted copy",snap:{cap:"The losing desktop edit is preserved as a new file row in the same namespace instead of clobbering <code>/plan.xlsx</code>. Its v1 history references <code>cf88</code>, and that chunk becomes live.",tables:[{name:"files (metadata shard ns42)",cols:["file_id","namespace_id","path","current_version","is_dir"],rows:[{c:["9001","42","/plan.xlsx","7","false"],tag:"winner kept"},{c:["9010","42","/plan (desktop conflicted copy).xlsx","1","false"],hi:1,tag:"conflicted copy"}]},{name:"file_versions (metadata shard ns42)",cols:["file_id","version","chunk_hashes","size","mtime","device_id"],rows:[{c:["9010","1","[c1a2, c9f3, cf88]","1053184","2026-07-25 07:40:22","5501"],hi:1,tag:"inserted"}]},{name:"chunks (dedup index)",cols:["chunk_hash","storage_url","ref_count","size"],rows:[{c:["cf88","s3://blk/cf/cf88","1","524912"],hi:1,tag:"ref +1"}]}]},narrate:"The desktop keeps the server's version as <code>/plan.xlsx</code> and writes its own edit as a new file path, for example <code>/plan (desktop conflicted copy).xlsx</code>. This is another normal metadata transaction in the same namespace shard.",details:[
      ...
      {node:"notif",title:"Notify devices of both durable results",snap:{cap:"The namespace now contains both durable outcomes. Subscribers at cursor 6 will pull a delta containing the v7 winner and the conflicted-copy file; no user bytes were silently lost.",tables:[{name:"files (metadata shard ns42)",cols:["file_id","namespace_id","path","current_version","is_dir"],rows:[{c:["9001","42","/plan.xlsx","7","false"],hi:1,tag:"winner"},{c:["9010","42","/plan (desktop conflicted copy).xlsx","1","false"],hi:1,tag:"conflict"}]},{name:"device_cursors (metadata shard ns42)",cols:["device_id","namespace_id","last_synced_version"],rows:[{c:["5502","42","7"],tag:"has winner"},{c:["5501","42","6"],hi:1,tag:"will pull both"}]}]},narrate:"Subscribers are poked once the namespace has the winner and the conflicted copy committed. Other devices pull the delta and see both files, preserving both users' work.",details:[
  ...
  steps:[
    {node:"db",stage:"Stage 0 · Baseline",title:"Whole-file upload — metadata and bytes stay simple",
      ...
      details:[
        ...
        {k:"pain",label:"What breaks — a concrete case",text:"At **Northstar Film Cut**, 180 editors share a project folder. Dana fixes one subtitle in a **2 GB** ProRes proxy at 09:12. The baseline client uploads the whole file through the API again, holding one gateway connection for minutes and writing another 2 GB version even though the changed region is a few KB. During the same morning, the platform is on track for **100M DAU × 100 changes/day = 10B change events/day**, **115K/s baseline** and **3−5× peak**. Whole-file upload turns harmless edits into a bandwidth and storage incident."},
        {k:"fix",label:"The fix — walk the same case, correct simple metadata",text:"For the MVP, keep the metadata CAS and make the failure mode explicit: Dana's new version advances the namespace cursor atomically, so every device eventually sees version 42 and no tree state is half-applied. This proves the correctness boundary, but it does not make the 2 GB retransmit acceptable. The next stage keeps the same metadata commit and changes the upload unit."},
        {k:"host",label:"Load & capacity — what runs it",text:"**CockroachDB metadata cluster**, 3 nodes across AZs, RF=3, stores namespaces, files, and file_versions; **API + metadata service** is stateless. A beta at **10K commits/s** with 3−5 rows/commit is ~50K row writes/s, fine for a small NewSQL cluster. Full peak is **~460K commits/s**, so a single 3-node group is not enough; the baseline is about correctness, and production must shard by namespace before accepting the global number."},
      ],
      snap:{title:"Load & capacity — Stage 0",cap:"The metadata control plane is clear, but one small edit still moves one whole file.",
        tables:[{name:"signals",cols:["signal","value","verdict"],rows:[
          {c:["Metadata store","CockroachDB RF=3 · 3 nodes · one range group","N+1 for beta"],hi:1},
          {c:["Change events","~115K/s baseline · ~460K/s peak","must shard"],hi:1},
          {c:["Northstar edit unit","2 GB file for one subtitle","waste"],tag:"risk"},
          {c:["Conflict rule","version CAS on file row","safe baseline"]},
          {c:["Byte path","through API gateway","next bottleneck"],hi:1,tag:"risk"},
        ]}] }},
    {node:"chunk",stage:"Stage 1 · Chunking",title:"One-byte edits re-upload files &rarr; add content-defined chunks",
      ...
      details:[
        {k:"pain",label:"What breaks — a concrete case",text:"Northstar's assistant inserts **3 seconds** of slate at the front of a 2 GB review file. Fixed 4 MB blocks make almost every downstream boundary shift, so the server thinks **~500 blocks** are new and the user re-uploads nearly 2 GB. At platform peak, even **1%** of the **460K changes/s** being 2 GB retransmits would imply **~9.2 PB/s** of pointless network. The metadata DB is fine; the byte protocol is bankrupt."},
        {k:"fix",label:"The fix — walk the same case, CDC plus dedup",text:"The client runs content-defined chunking with a **~4 MB target** and strong hashes before upload. The slate insert disturbs the first chunk and then boundaries re-align, so Dana uploads roughly **one 4 MB chunk**, not 2 GB. At **500 MB/s local hashing**, scanning the 2 GB file takes ~4 s on the workstation, while transfer falls by **~99.8%**. The server checks which hashes already exist and receives only misses."},
        {k:"host",label:"Load & capacity — what runs it",text:"**Client-side chunker + hash index**, target 4 MB chunks; **dedup lookup service** backed by a sharded RocksDB/Scylla hash index. Peak change rate **460K/s × average 1 MB** would require ~460 GB/s server hashing if centralized, about **460 CPU cores at 1 GB/s/core**; pushing hashing to clients removes that tier. The server index only answers hash-exists checks and verifies bytes after upload."},
      ...
      snap:{title:"Load & capacity — Stage 1",cap:"The upload unit becomes changed chunks, not changed files.",
        tables:[{name:"signals",cols:["signal","before","after"],rows:[
          {c:["Northstar tiny edit","2 GB uploaded","~4 MB uploaded"],hi:1,tag:"fixed"},
          {c:["Chunk target","not applicable","~4 MB content-defined chunks"]},
          {c:["Hash engine","central byte tier","client CPU · ~500 MB/s each"],hi:1},
          {c:["Server CPU avoided","~460 cores at 460 GB/s peak","pushed to clients"],hi:1},
          {c:["Dedup index","none","Scylla/RocksDB hash shards · RF=3"]},
        ]}] }},
    {node:"block",stage:"Stage 2 · Block store",title:"Chunks outgrow the DB &rarr; move bytes to block storage",
      ...
      details:[
        {k:"pain",label:"What breaks — a concrete case",text:"Northstar's folder has 70 TB of media, but the real platform projects **~25 EB raw**, **~12 EB unique after dedup**, and **~18 EB physical with erasure coding**. If chunks live in CockroachDB beside `files` rows, compaction, backups, and range splits drag huge byte values through the strong metadata path. A few 50 GB uploads fill raft logs and the 3−5 row namespace commit that should take milliseconds waits behind object bytes."},
        {k:"fix",label:"The fix — walk the same case, split control and data planes",text:"Dana's client uploads missing chunks directly to an S3-like block store using scoped URLs. Only after the chunk is durable and hash-verified does metadata commit the manifest and advance the namespace cursor. The visible file version points to immutable content-addressed objects; if metadata commit fails, the orphan chunk is garbage-collected. The 70 TB Northstar folder now costs object-store capacity, not NewSQL write latency."},
        {k:"host",label:"Load & capacity — what runs it",text:"**Object/block store** with Reed-Solomon **EC 10+4** across storage nodes: 12 EB unique becomes roughly **16.8 EB** before metadata and repair overhead, matching the file's **~18 EB physical** claim. Use thousands of storage nodes, e.g. **9,000 nodes × 2 PB usable ≈ 18 EB**. Metadata remains **CockroachDB/Spanner**, sharded by namespace; **1,500 metadata nodes × 2 TB usable ≈ 3 PB replicated** covers ~5T version rows at ~200 B each ×3."},
      ...
      snap:{title:"Load & capacity — Stage 2",cap:"The strong store now holds pointers; the block plane scales for exabytes.",
        tables:[{name:"signals",cols:["signal","value","verdict"],rows:[
          {c:["Block store","S3-like object store · EC 10+4 · ~9K storage nodes","enough for ~18 EB"],hi:1,tag:"fixed"},
          {c:["Raw user bytes","~25 EB","block plane"]},
          {c:["Unique after dedup","~12 EB","content addressed"]},
          {c:["Metadata DB","CockroachDB/Spanner · ~1,500 nodes · RF=3","~3 PB replicated"],hi:1},
          {c:["Commit order","chunk durable &rarr; manifest visible","no missing bytes"]},
        ]}] }},
    {node:"notif",stage:"Stage 3 · Notifications",title:"Devices poll for changes &rarr; push version pokes",
      ...
      details:[
        {k:"pain",label:"What breaks — a concrete case",text:"Northstar shares the cut folder with **5,000 collaborators**. After Dana commits version 42, every desktop keeps polling once a minute to ask `changes-since 41`. Platform-wide, **100M devices ÷ 60 s ≈ 1.6M reads/s** hit metadata even when nothing changed. At Northstar, a single commit creates 5,000 identical reads in the same few seconds; without caching and pokes, sync feels delayed and the metadata read replicas get a herd for one cursor answer."},
        {k:"fix",label:"The fix — walk the same case, best-effort poke and authoritative pull",text:"Metadata writes one namespace-advanced event after version 42 commits. The notification service sends a tiny poke to Northstar's connected devices with jitter over **2−5 s**; each device then pulls `changes-since 41`, and the gateway caches that immutable response. The first request fills the cache; the other 4,999 get the same manifest delta without touching CockroachDB. A missed poke is safe because periodic cursor polling remains the slow backstop."},
        {k:"host",label:"Load & capacity — what runs it",text:"**Kafka** carries namespace events with **256 partitions by namespace_id**. At **460K peak commits/s**, partitions see **~1.8K events/s** each; event size is a few hundred bytes, so partitions are for ordering and consumer parallelism, not bytes. **Notification gateways** maintain WebSockets/long-polls; at **50K connections/node**, 100M connected devices would be ~2,000 nodes globally, typically regionally split. Metadata read replicas serve cache misses only."},
      ...
      snap:{title:"Load & capacity — Stage 3 (full design)",cap:"Sync becomes event-driven while correctness remains cursor-based.",
        tables:[{name:"signals",cols:["concern","mechanism","result"],rows:[
          {c:["Event bus","Kafka · 256 partitions by namespace_id · RF=3","ordered + parallel"],hi:1,tag:"fixed"},
          {c:["Partition math","460K commits/s ÷ 256 ≈ 1.8K events/partition","headroom"]},
          {c:["Cold polling","~1.6M reads/s","collapsed by pokes + cache"],hi:1},
          {c:["Northstar herd","5,000 identical changes-since reads","one cached response"],hi:1},
          {c:["Correctness source","metadata cursor","lost poke self-heals"]},
        ]}] }},
  ]};
